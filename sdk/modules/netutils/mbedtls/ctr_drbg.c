...
int mbedtls_ctr_drbg_seed( mbedtls_ctr_drbg_context *ctx,
                   ...
                   size_t len )
{
    return( mbedtls_ctr_drbg_seed_entropy_len( ctx, mbedtls_entropy_func, p_entropy, custom, len,
...
int mbedtls_ctr_drbg_write_seed_file( mbedtls_ctr_drbg_context *ctx, const char *path )
{
    ...
    if( ( f = FM_FileOpen( path, FM_MODE_WRITE) ) == NULL )
        return( MBEDTLS_ERR_CTR_DRBG_FILE_IO_ERROR );

    if( ( ret = mbedtls_ctr_drbg_random( ctx, buf, MBEDTLS_CTR_DRBG_MAX_INPUT ) ) != 0 )
        goto exit;

    if( FM_FileWrite( buf, MBEDTLS_CTR_DRBG_MAX_INPUT, f ) != MBEDTLS_CTR_DRBG_MAX_INPUT )
    {
...
int mbedtls_ctr_drbg_update_seed_file( mbedtls_ctr_drbg_context *ctx, const char *path )
{
...
#if defined(CONFIG_OS_MERLOT)
    FM_FILE  *f;
    if( ( f = FM_FileOpen( path, FM_MODE_READ) ) == NULL )
        return( MBEDTLS_ERR_CTR_DRBG_FILE_IO_ERROR );

    n = FM_FileSeek(0, FM_SEEK_END , f);
    FM_FileSeek(0, FM_SEEK_START , f);
    ...
    if( FM_FileRead( buf, (uint32_t *)&n, f ) != 0 )
    {
        FM_FileClose( f );
        return( MBEDTLS_ERR_CTR_DRBG_FILE_IO_ERROR );
    }
    ...
    n = 0;
    while( (ret=fread( &tmp_buf, 1, 1, f)) > 0 )
    {
      n++;
    }
