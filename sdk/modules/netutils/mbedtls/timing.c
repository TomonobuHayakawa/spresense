...
#include "netutils/mbedtls/timing.h"
...
struct timezone {
    int tz_minuteswest;     /* グリニッジ標準時との差 */
    int tz_dsttime;         /* 夏時間調整の型 */
};

int
gettimeofday(struct timeval *tval, struct timezone *tz)
{
    ...
    { 
		time_t nowTick = 0;
		SYS_GetTime((SYS_Time*)&nowTick);

		tval->tv_sec = nowTick / 1000; 
		tval->tv_usec = nowTick % 1000 * 1000; 
	} 
