...
#include "netutils/mbedtls/debug.h"
...
#if defined(MBEDTLS_HAVE_TIME)
#if defined(CONFIG_OS_MERLOT)
	SYS_RtcMgrGetTime(MBEDDTSL_RTC_CH, &t);
#elif defined(CONFIG_OS_NUTTX)
...
#if defined(MBEDTLS_HAVE_TIME)
#if defined(CONFIG_OS_MERLOT)
		SYS_RtcMgrGetTime(MBEDDTSL_RTC_CH, &ssl->session_negotiate->start);
#elif defined(CONFIG_OS_NUTTX)
		clock_gettime( CLOCK_MONOTONIC, &tp );
		ssl->session_negotiate->start=tp.tv_sec;
#else
                                    ...
                                    size_t pms_offset )
{
    ...
    unsigned char mask;
    size_t i, peer_pmslen = 0;
    unsigned int diff;

    memset(peer_pms, 0, sizeof(peer_pms));
