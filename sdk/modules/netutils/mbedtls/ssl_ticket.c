                               ...
                               unsigned char index )
{
	...
	SYS_RtcMgrGetTime(MBEDDTSL_RTC_CH, &key->generation_time);
#elif defined(CONFIG_OS_NUTTX)
	struct timespec tp;
	clock_gettime( CLOCK_MONOTONIC, &tp );
	key->generation_time = tp.tv_sec;
#else
...
static int ssl_ticket_update_keys( mbedtls_ssl_ticket_context *ctx )
{
    ...
    {
#if defined(CONFIG_OS_MERLOT)
		SYS_RtcTime_t current_time;
		SYS_RtcMgrGetTime(MBEDDTSL_RTC_CH, &current_time);
#elif defined(CONFIG_OS_NUTTX)
		time_t current_time;
		struct timespec tp;
		clock_gettime( CLOCK_MONOTONIC, &tp );
