...
#ifndef __CYGWIN__
#ifdef SPZ2_IMPL
    //PTHREAD_MUTEX_RECURSIVE_NP is not defined by NuttX.
...
/*******************************************************************************
**
** Function         GKI_destroy
**
** Description      Destroy GKI_CB
**
** Returns          void
**
*******************************************************************************/

void GKI_destroy (void)
{
#ifdef SPZ_IMPL
...
#ifndef SPZ2_IMPL
    //PTHREAD_CREATE_DETACHED is not defined by NuttX.
...
#if GKI_BUFFER_DEBUG
    GKI_dump_buffer_pools();
#endif

/* gki_delete_pool begin */
#if (defined GKI_USE_DEFERED_ALLOC_BUF_POOLS && !defined GKI_DELETE_POOL_DISABLE)
    /* free memory */
    GKI_delete_pools();
#endif
/* gki_delete_pool end */
    ...
    {
        GKI_TRACE_1("GKI_shutdown :  release_wake_lock(%s)", WAKE_LOCK_ID);
#ifdef ANDROID
...
#if GKI_DYNAMIC_MEMORY == TRUE
    int n=0;
    while ((*p_run_cond != GKI_TIMER_TICK_NONE_COND) && (++n < 10))
        usleep(100 * 1000);
#endif
    GKI_destroy();
}
...
void GKI_run (void *p_task_id)
{
    ...
    GKI_TRACE_0( "GKI_run(): Start/Stop GKI_timer_update_registered!" );
#endif
...
#endif
    *p_run_cond = GKI_TIMER_TICK_NONE_COND;
    GKI_TRACE_1("%s exit", __func__);
}
...
/* gki_delete_pool begin */
#if (!defined GKI_DELETE_POOL_DISABLE)
    if(code == GKI_ERROR_DELETE_POOL_BAD_QID)
        return;
#endif
/* gki_delete_pool end */
