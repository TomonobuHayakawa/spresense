...
/*******************************************************************************
**
** Function         gki_dump_mem
**
** Description      dump memory data
**
** Parameters       disp_str - (input) specify string. it will be displayed in log.
**                  p        - (input) memory address that you want to display in log.
**                  len      - (input) memory total length in bytes.
**                  disp_len - (input) memory length in bytes that you want to display in log.
**
** Returns          void
**
*******************************************************************************/
void gki_dump_mem(const char *disp_str, UINT8 *p, UINT16 len, UINT16 disp_len)
{
#define GKI_DUMP_MEM_BYTES_PER_LINE    16
#define GKI_DUMP_MEM_LEN_PER_BYTE       3   /* strlen("HH ") */
UINT16 i;
UINT16 len1;
UINT16 len2;
UINT8 buf0[GKI_DUMP_MEM_LEN_PER_BYTE+1];
UINT8 buf1[GKI_DUMP_MEM_BYTES_PER_LINE*GKI_DUMP_MEM_LEN_PER_BYTE+1];
    ...
    if(p == NULL || len == 0)
    {
        LOGD("%s dump(ptr=%p,len=%d/%d)", disp_str, p, len, disp_len);
        return;
    }
    else
    {
        ...
        while(len1 > 0)
        {
            if(len1 >= GKI_DUMP_MEM_BYTES_PER_LINE)
            {
                len2 = GKI_DUMP_MEM_BYTES_PER_LINE;
            }
            else
            {
                len2 = len1;
            ...
            for(i=0;i<len2;i++)
            {
                snprintf(buf0, sizeof(buf0), "%02X ", *p);
                p++;
                strcat(buf1, buf0);
            }
            LOGD("%s     %s", disp_str, buf1);
            len1 -= len2;
        }
    }
}
/*******************************************************************************
**
** Function         GKI_dump_buffer_pools
**
** Description      dump remaining memory in memory pools
**
** Parameters:      void
**
** Returns          void
**
*******************************************************************************/
void GKI_dump_buffer_pools(void)
{
    UINT8         i;
    BUFFER_HDR_T  *p_hdr;
    tGKI_COM_CB *p_cb = &gki_cb.com;
    ...
    for (i=0 ; i < p_cb->curr_total_no_of_pools; i++)
    {
        ...
        for (x=0; p_hdr && x < p_cb->freeq[i].total; x++)
        {
            ...
            {
                LOGD("pool:%d, buf[%d]:%x, hdr:%x status=%d func:%s(line=%d)", i, x, (UINT8*)p_hdr + BUFFER_HDR_SIZE, p_hdr, p_hdr->status, p_hdr->_function, p_hdr->_line);
                gki_dump_mem("", (UINT8*)p_hdr + BUFFER_HDR_SIZE, p_cb->pool_size[i] - BUFFER_HDR_SIZE, 32);
            }
            p_hdr = (BUFFER_HDR_T *)((UINT8 *)p_hdr + p_cb->pool_size[i]);
        }
    }
...
#endif /* GKI_BUFFER_DEBUG */
/* gki_delete_pool begin */
/*******************************************************************************
**
** Function         GKI_delete_pools
**
** Description      delete all memory pools that are allocated dynamically.
**
** Parameters:      void
**
** Returns          void
**
*******************************************************************************/
void GKI_delete_pools ()
{
#if (defined GKI_USE_DEFERED_ALLOC_BUF_POOLS || (GKI_USE_DYNAMIC_BUFFERS == TRUE))
    tGKI_COM_CB *p_cb = &gki_cb.com;

#if (GKI_NUM_FIXED_BUF_POOLS > 0)
    GKI_delete_pool(GKI_POOL_ID_0);
#endif

#if (GKI_NUM_FIXED_BUF_POOLS > 1)
    GKI_delete_pool(GKI_POOL_ID_1);
#endif

#if (GKI_NUM_FIXED_BUF_POOLS > 2)
    GKI_delete_pool(GKI_POOL_ID_2);
#endif

#if (GKI_NUM_FIXED_BUF_POOLS > 3)
    GKI_delete_pool(GKI_POOL_ID_3);
#endif

#if (GKI_NUM_FIXED_BUF_POOLS > 4)
    GKI_delete_pool(GKI_POOL_ID_4);
#endif

#if (GKI_NUM_FIXED_BUF_POOLS > 5)
    GKI_delete_pool(GKI_POOL_ID_5);
#endif

#if (GKI_NUM_FIXED_BUF_POOLS > 6)
    GKI_delete_pool(GKI_POOL_ID_6);
#endif

#if (GKI_NUM_FIXED_BUF_POOLS > 7)
    GKI_delete_pool(GKI_POOL_ID_7);
#endif

#if (GKI_NUM_FIXED_BUF_POOLS > 8)
    GKI_delete_pool(GKI_POOL_ID_8);
#endif

#if (GKI_NUM_FIXED_BUF_POOLS > 9)
    GKI_delete_pool(GKI_POOL_ID_9);
#endif

#if (GKI_NUM_FIXED_BUF_POOLS > 10)
    GKI_delete_pool(GKI_POOL_ID_10);
#endif

#if (GKI_NUM_FIXED_BUF_POOLS > 11)
    GKI_delete_pool(GKI_POOL_ID_11);
#endif

#if (GKI_NUM_FIXED_BUF_POOLS > 12)
    GKI_delete_pool(GKI_POOL_ID_12);
#endif

#if (GKI_NUM_FIXED_BUF_POOLS > 13)
    GKI_delete_pool(GKI_POOL_ID_13);
#endif

#if (GKI_NUM_FIXED_BUF_POOLS > 14)
    GKI_delete_pool(GKI_POOL_ID_14);
#endif

#if (GKI_NUM_FIXED_BUF_POOLS > 15)
    GKI_delete_pool(GKI_POOL_ID_15);
#endif

#endif /* (defined GKI_USE_DEFERED_ALLOC_BUF_POOLS || (GKI_USE_DYNAMIC_BUFFERS == TRUE)) */
}
/* gki_delete_pool end */
