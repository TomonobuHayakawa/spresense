...
{
    int retval = 0;
    const cxd224x_dev_t *dev = (const cxd224x_dev_t*) p_dev;
...
{
    int retval = 0;
    cxd224x_dev_t* dev = (cxd224x_dev_t*) p_dev;
...
static int nfc_open (const hw_module_t* module, const char* name, hw_device_t** device)
{
    ...
    if (strncmp (name, NFC_NCI_CONTROLLER, sizeof(NFC_NCI_CONTROLLER)) == 0)
    {
        cxd224x_dev_t *dev = calloc (1, sizeof(cxd224x_dev_t));
...
struct nfc_nci_module_t nfc_nci_hw = //HAL_MODULE_INFO_SYM =
{
    .common =
    {
