...
tNFC_STATUS rw_i93_get_next_blocks (UINT16 offset)
{
    ...
    if (p_i93->intl_flags & RW_I93_FLAG_READ_MULTI_BLOCK)
    {
        ...
        /* CXD224x workaround: block read limitation */
        if (I93_CXD_MAX_READ_BLOCK_SIZE < RW_I93_READ_MULTI_BLOCK_SIZE &&
            GetNumValue(NAME_CXD224X_WORKAROUND, &num, sizeof(num)) && (num & NFC_CXD_WORKAROUND_ISO15693_MULBLK))
        {
            if (num_block > I93_CXD_MAX_READ_BLOCK_SIZE/p_i93->block_size)
            {
                num_block = I93_CXD_MAX_READ_BLOCK_SIZE/p_i93->block_size;
            }
        }
...
tNFC_STATUS rw_i93_get_next_block_sec (void)
{
    ...
    /* CXD224x workaround: block read limitation */
    if (GetNumValue(NAME_CXD224X_WORKAROUND, &num, sizeof(num)) && (num & NFC_CXD_WORKAROUND_ISO15693_MULBLK))
    {
        if (num_blocks > I93_CXD_MAX_READ_BLOCK_SIZE)
        {
            num_blocks = I93_CXD_MAX_READ_BLOCK_SIZE;
        }
    }
...
void rw_i93_sm_detect_ndef (BT_HDR *p_resp)
{
    ...
    case RW_I93_SUBSTATE_CHECK_LOCK_STATUS:
        ...
        else
        {
            ...
            last_block  = (p_i93->ndef_tlv_last_offset / p_i93->block_size);
            /* CXD224x workaround: if FrameRF interface is usded, status byte is added in the end of resp.
             * in this case length is not equal num_blocks, p_i93->num_block should be used */
            num_blocks  = p_i93->num_block < length ? p_i93->num_block : length;
...
static void rw_i93_data_cback (UINT8 conn_id, tNFC_CONN_EVT event, tNFC_CONN *p_data)
{
    ...
    /* CXD224x workaround: if FrameRF interface is used, status byte was checked and update data.status */
    {
        if (p_data->data.status != NFC_STATUS_OK)
        {
            if(p_data->data.p_data != NULL)
            {
                GKI_freebuf(p_data->data.p_data);
                p_data->data.p_data = NULL;
            }

            RW_TRACE_DEBUG1 ("rw_i93_data_cback () state=%d", p_i93->state);
            if ( (p_i93->state != RW_I93_STATE_NOT_ACTIVATED)
                    && (p_i93->state != RW_I93_STATE_IDLE)  )
            {
                RW_TRACE_ERROR1 ("rw_i93_data_cback (): data packets error status = %d", p_data->data.status);
