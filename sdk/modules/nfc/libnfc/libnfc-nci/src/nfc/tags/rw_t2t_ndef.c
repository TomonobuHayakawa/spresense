...
/* Tag2 Tag Operation Specification: 2.2.2 Static and Dynamic Lock Bit    */
/* NumberOfDynamicLockByte = (DataAreaSize -48)/64                        */
/* MaxDynamicLockByte = ((T2T_SECTOR_SIZE * T2T_MAX_SECTOR) - 48)/64      */
/*                    = 31.25 => 32                                       */
#ifndef T2T_MAX_DYNAMIC_LOCKBYTE
#define T2T_MAX_DYNAMIC_LOCKBYTE 32
#endif
...
static void rw_t2t_handle_tlv_detect_rsp (UINT8 *p_data)
{
    ...
    if (!found && !failed)
    {
        ...
        {
            /* LIBNCI bugfix: if NULL TLV crossing 16byte boundary, should not add T2T_FIRST_DATA_BLOCK to read len*/
            int num=0;
            if( GetNumValue(NAME_LIBNCI_BUGFIX, &num, sizeof(num)) && (num & NFC_LIBNCI_BUGFIX_T2TNULLTLV))
            {
...
static UINT16 rw_t2t_get_ndef_max_size (void)
{
    ...
    /* DTA workaround TC_T2T_NDA_BV_3_x INITIALIZED T2T Dynamic Memory Layout */
    /* total size is (T2T_SECTOR_SIZE * T2T_MAX_SECTOR + lock byte(8block))   */
    if (  (tag_size < T2T_STATIC_SIZE)
        ||(tag_size > ((T2T_SECTOR_SIZE * T2T_MAX_SECTOR) + T2T_MAX_DYNAMIC_LOCKBYTE))
