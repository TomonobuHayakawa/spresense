...
#include <stdlib.h>
#include <string.h>
#include <sys/types.h>   /* abort()/kill() */
#include <unistd.h>      /* abort()/kill() */
#include <stdlib.h>      /* abort()/kill() */
#include <signal.h>
...
void nfc_ncif_halcc_callback (tNFC_HALCC_EVT event, UINT16 data_len, UINT8 *p_data)
{
    NFC_TRACE_ERROR0 ("nfc_ncif_halcc_callback");
    kill(getpid(), SIGTERM);
}
...
void nfc_ncif_cmd_timeout (void)
{
    ...
    if (GetNumValue (NAME_SELECT_PANIC_ACTION, &num, sizeof(num)) && num)
    {
        ...
        switch (num)
        {
        case NFC_SELECT_PANIC_ACTION_EVENT:
            NFC_TRACE_ERROR0 ("NFC controller stopped responding, go to abort()");
            break;
        case NFC_SELECT_PANIC_ACTION_RSTKILL:
        case NFC_SELECT_PANIC_ACTION_RSTEVENT:
#if (defined (NFA_HALCC_INCLUDED) && (NFA_HALCC_INCLUDED==TRUE))
            NFC_TRACE_ERROR0 ("NFC controller stopped responding, Rest Device and killing the NFC process");
            status = NFC_SendHalControlCommand(NFC_HAL_DEVICE_RESET, NULL, nfc_ncif_halcc_callback);
            if (status == NFC_STATUS_OK) return;
            break;
#endif /*(defined (NFA_HALCC_INCLUDED) && (NFA_HALCC_INCLUDED==TRUE))*/
            /* if NFA_HALCC_INCLUDED==FALSE, continue */
        case NFC_SELECT_PANIC_ACTION_KILL:
            NFC_TRACE_ERROR0 ("NFC controller stopped responding, killing the NFC process");
            ret = kill(getpid(), SIGTERM);
            if (!ret) return;
            break;
        case NFC_SELECT_PANIC_ACTION_EVENTRET:
            /* for debug use only */
            NFC_TRACE_ERROR0 ("NFC controller stopped responding, exit. should be handing GEN_ERROR(HW_TIMEOUT)");
...
HRx 0 or 2 Octets   If present, the first byte SHALL contain HR0 and the second byte SHALL contain HR1 as defined in [DIGITAL].
        */
        ...
        if (u8)
            ...
            p_pa->sel_rsp    = 0; /* LIBNCI bugfix: clear if sel_rsp not present */
...
void nfc_ncif_proc_reset_rsp (UINT8 *p, BOOLEAN is_ntf)
{
    ...
    if (is_ntf)
    {
        ...
        if (GetNumValue (NAME_SELECT_PANIC_ACTION, &num, sizeof(num)) && num)
        {
            ...
            {
            case NFC_SELECT_PANIC_ACTION_RSTKILL:
                /* Do not assert reset, when recv CORE_RESET_NTF */
                /* continue */
            case NFC_SELECT_PANIC_ACTION_KILL:
                ret = kill (getpid(), SIGTERM);
                if (!ret) break;
                /* continue */
            case NFC_SELECT_PANIC_ACTION_ABORT:
                abort();
                break;
            case NFC_SELECT_PANIC_ACTION_EVENTRET:
            case NFC_SELECT_PANIC_ACTION_RSTEVENT:
            case NFC_SELECT_PANIC_ACTION_EVENT:
            default:
                evt_data.status =  status; /* Reason Code */
                if (p_cback)
                {
                    (*p_cback) (NFC_CORE_RESET_NTF_REVT, &evt_data);
                }
            }
            return;
        }
    ...
    /* LIBNCI bugfix: the second byte in the CORE_RESET_NTF is not version */
    if (status == NCI_STATUS_OK && !is_ntf)
    {
        if ((*p) != NCI_VERSION)
        {
    ...
    if (p_cb->p_cback)
    {
        while ((p_evt = (BT_HDR *)GKI_getfirst (&p_cb->rx_q)) != NULL)
        {
            if (p_evt->layer_specific & NFC_RAS_FRAGMENTED)
            {
                ...
                {
                    /* buffer can hold more */ /* if assembly T3T packet, enable comment-outed region in the following line */
                    if (  (p_cb->conn_id != NFC_RF_CONN_ID) /*|| (p_cb->act_protocol == NCI_PROTOCOL_T3T)*/
                        ||(nfc_cb.reassembly)  )
                    {
            ...
            if (p_cb->conn_id == NFC_RF_CONN_ID
                && (data_cevt.status != NFC_STATUS_CONTINUE)) /*LIBNCI bugfix: if cont, don't remove status*/
            {
                unsigned long num=0;
                BOOLEAN flag_iso15693=FALSE;

                /* if NCI_PROTOCOL_15693, the status byte needs to be removed */
                if( GetNumValue(NAME_CXD224X_WORKAROUND, &num, sizeof(num)) && (num & NFC_CXD_WORKAROUND_ISO15693) )
                {
                 ...
                 */
                if (((p_cb->act_protocol >= NCI_PROTOCOL_T1T) && (p_cb->act_protocol <= NCI_PROTOCOL_T3T)) || flag_iso15693)
                {
                    p_evt->len--;
                    p                = (UINT8 *) (p_evt + 1);
...
void nfc_ncif_proc_data (BT_HDR *p_msg)
{
    ...
    if (pp[0]==0 && pp[1]==0 && pp[2]==0) /* CXD224x workaround: drop all zero packet start */
    {
        unsigned int num=0;
        if( GetNumValue(NAME_CXD224X_WORKAROUND, &num, sizeof(num)) && (num & NFC_CXD_WORKAROUND_DROP_ALLZERO_PKT) )
        {
            NFC_TRACE_DEBUG0 ("nfc_ncif_proc_data drop all zero packet");
            GKI_freebuf (p_msg);
            return;
        }
    } /* CXD224x workaround: drop all zero packet end */
