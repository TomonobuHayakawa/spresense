...
#if (NFC_RW_ONLY == FALSE) && (((defined (NFA_P2P_INCLUDED) && NFA_P2P_INCLUDED == TRUE))||(!defined (NFA_P2P_INCLUDED)))
#include "ce_api.h"
#include "ce_int.h"
#include "llcp_int.h"
...
#if (NFC_RW_ONLY_PLUS_CE == TRUE) || (NFA_CE_INCLUDED ==TRUE)
#include "ce_api.h"
#include "ce_int.h"
#else /* NFC_RW_ONLY_PLUS_CE */
#define ce_init()
#endif /* NFC_RW_ONLY_PLUS_CE */
...
#include <assert.h>
    nfc_cb_ptr = (tNFC_CB *)GKI_os_malloc( sizeof(tNFC_CB) );
    assert (nfc_cb_ptr);
#endif
...
**
*******************************************************************************/
tNFC_STATUS NFC_ConnDummyCreate (UINT8 conn_id)
{

    tNFC_CONN_CB *p_conn_cb = nfc_alloc_conn_cb (NULL);
    tNFC_STATUS     status = NFC_STATUS_FAILED;

    if (p_conn_cb)
    {
        nfc_set_conn_id (p_conn_cb, conn_id);
