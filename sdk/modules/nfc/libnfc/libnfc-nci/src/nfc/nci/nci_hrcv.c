...
**
*******************************************************************************/
void nci_proc_halcc_rsp (BT_HDR *p_msg)
{
    UINT8   *p, op_code;
    UINT16 len;
    tNFC_HALCC_CBACK   *p_cback = (tNFC_HALCC_CBACK *)nfc_cb.p_halcc_cb;

    NFC_TRACE_DEBUG0("nci_proc_halcc_rsp()");
    ...
    if( p_cback )
    {
        ...
        (*p_cback) ( op_code, len, p );
        nfc_cb.p_halcc_cb = NULL;
    }
    return ;
}
