...
tNFC_ADAPTATION_DM_CBACK* NfcAdaptation::mDmCallback = NULL;
tNFC_ADAPTATION_DEVINFO_CBACK* NfcAdaptation::mDevInfoCallback = NULL;
...
void NfcAdaptation::Initialize ()
{
    ...
    {
        const char default_nfc_location[] = "/data/nfc";
        memset (bcm_nfc_location, 0, sizeof(bcm_nfc_location));
        strncpy (bcm_nfc_location, default_nfc_location, sizeof(default_nfc_location));
    }
...
void NfcAdaptation::Finalize()
{
    ...
    ALOGD ("%s: enter", func);
    GKI_shutdown ();
#ifndef SPZ2_IMPL //TODO Support Config file
    resetConfig();
#endif
...
void NfcAdaptation::InitializeHalDeviceContext ()
{
    ...
    if ( !GetStrValue ( NAME_NCI_HAL_MODULE, nci_hal_module, sizeof ( nci_hal_module) ) )
    {
        ...
        memset (nci_hal_module, 0, sizeof(nci_hal_module));
        strncpy (nci_hal_module, "nfc_nci.bcm2079x", sizeof(nci_hal_module));
#else
        char nci_hal_modu[] = "nfc_nci.cxd224x";
        memcpy(nci_hal_module, nci_hal_modu, sizeof(nci_hal_module));
#endif /* SPZ_IMPL */
    }
...
{
    const char* func = "NfcAdaptation::HalGetMaxNfcee"; /* LIBNCI fixtypo: */
    UINT8 maxNfcee = 0;
    ALOGD ("%s", func);
    if (mHalDeviceContext)
    {
    ...
    case HAL_NFC_OPEN_CPLT_EVT:
        {
            ...
            if (NfcAdaptation::mDmCallback)
            {
                NfcAdaptation::mDmCallback(NFA_DM_ENABLE_EVT, (unsigned int )event_status);
            }
...
void NfcAdaptation::HalDownloadFirmwareDataCallback (uint16_t data_len, uint8_t* p_data)
{
    ...
    ALOGD ("%s: data_len=%d", func, data_len);

    if (data_len < NCI_MSG_HDR_SIZE) return;
    ...
    if (mt == NCI_MT_HALMSG && cmd == NFC_HAL_GET_DEVICE_INFO)
    {
        if (NfcAdaptation::mDevInfoCallback && data_len)
            NfcAdaptation::mDevInfoCallback (cmd, data_len, pp);
        NfcAdaptation::mDevInfoCallback = NULL;
    }
}
...
**
*******************************************************************************/
void NfcAdaptation::DownloadFirmwareGetDeviceInfo (tNFC_ADAPTATION_DM_CBACK*      p_dm_cback,
                                                   tNFC_ADAPTATION_DEVINFO_CBACK* p_devinfo_cback)
{
    const char* func = "NfcAdaptation::DownloadFirmware";
    ALOGD ("%s: enter", func);
    HalInitialize ();
    ...
    mHalOpenCompletedEvent.lock ();
    NfcAdaptation::mDmCallback = p_dm_cback;
    NfcAdaptation::mDevInfoCallback = p_devinfo_cback;
    ALOGD ("%s: try open HAL", func);
    HalOpen (HalDownloadFirmwareCallback, HalDownloadFirmwareDataCallback);
    mHalOpenCompletedEvent.wait ();
    ...
    NfcAdaptation::mDmCallback = NULL;
    NfcAdaptation::mDevInfoCallback = NULL;
