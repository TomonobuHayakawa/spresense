...
void nfc_ce_t3t_set_listen_params (void)
{
    UINT8 i;
    tNFA_CE_CB *p_cb = &nfa_ce_cb;
    UINT8 tlv[((1+1+2+NCI_RF_F_UID_LEN)*NFA_CE_LISTEN_INFO_MAX) + (1+1+2)], *p_params; /* LIBNCI bugfix: */
    UINT8 tlv_size;
    ...
    /* CXD224x workaround: F/W BUG fix bigendian packet*/
    if( GetNumValue(NAME_CXD224X_WORKAROUND, &num, sizeof(num)) && (num & NFC_CXD_WORKAROUND_ENDIAN_ERRATA) )
    {
        UINT16_TO_BE_STREAM (p_params, t3t_flags2_mask);            /* Mask of IDs to disable listening */
    }
    else
    {
#ifndef SPZ_IMPL
...
BOOLEAN nfa_ce_activate_ntf (tNFA_CE_MSG *p_ce_msg)
{
    ...
    UINT8 listen_info_idx_candidate = NFA_CE_LISTEN_INFO_IDX_INVALID; /* tempworkaround: HCEF has priority to T3T UICC*/
    UINT8 *p_nfcid2 = NULL;
    UINT8 i;
        ...
        for (listen_info_idx=0; listen_info_idx<NFA_CE_LISTEN_INFO_IDX_INVALID; listen_info_idx++)
        {
            ...
            if (p_cb->listen_info[listen_info_idx].flags & NFA_CE_LISTEN_INFO_IN_USE)
            {
                if (p_cb->listen_info[listen_info_idx].protocol_mask & NFA_PROTOCOL_MASK_T3T)
                {
                    ...
                    /* CXD224x workaround: Bugfix T3T and Frame RF Interface in RF_INTF_ACTIVATE_NTF, copy local nfcid2 */
                    if (GetNumValue(NAME_CXD224X_WORKAROUND, &num, sizeof(num)) && (num & NFC_CXD_WORKAROUND_ACT_NTF_NFCID2))
                    {
                        if ((p_cb->activation_params.rf_tech_param.param.lf.nfcid2[0] == 0x00)
                            && (p_cb->activation_params.rf_tech_param.param.lf.nfcid2[1] == 0x00))
                        {
                            memcpy(p_cb->activation_params.rf_tech_param.param.lf.nfcid2, p_nfcid2, NCI_RF_F_UID_LEN);
                        }
                    }
#ifdef SPZ_IMPL
                    else {
                    ...
                    /* tempworkaround: HCEF has priority to T3T UICC start */
                    if (p_cb->listen_info[listen_info_idx].flags & NFA_CE_LISTEN_INFO_FELICA_HCEF)
                    {
                        break;
                    }
                    /* tempworkaround: HCEF has priority to T3T UICC end */
                }
                ...
                {
//                    break; /* do not break, cont. to seek HCEF *//* tempworkaround: HCEF has priority to T3T UICC*/
                    listen_info_idx_candidate = listen_info_idx; /* tempworkaround: HCEF has priority to T3T UICC*/
                }
            }
        }
        /* tempworkaround: HCEF has priority to T3T UICC start*/
        if ( listen_info_idx == NFA_CE_LISTEN_INFO_IDX_INVALID && listen_info_idx_candidate != NFA_CE_LISTEN_INFO_IDX_INVALID)
            listen_info_idx = listen_info_idx_candidate; /* if not found HCEF */
        NFA_TRACE_API1 ("nfa_ce_activate_ntf: detect entry=%d", listen_info_idx);
        /* tempworkaround: HCEF has priority to T3T UICC end*/
...
BOOLEAN nfa_ce_deactivate_ntf (tNFA_CE_MSG *p_ce_msg)
{
    ...
    for (i=0; i<NFA_CE_LISTEN_INFO_IDX_INVALID; i++)
    {
        if (p_cb->listen_info[i].flags & NFA_CE_LISTEN_INFO_IN_USE)
        {
            ...
            {
                conn_evt.deactivated.type =  deact_type;
                if (p_cb->p_active_conn_cback) /* add NULL check */
                    (*p_cb->p_active_conn_cback) (NFA_DEACTIVATED_EVT, &conn_evt);
            }
            else if (  (p_cb->activation_params.protocol == NFA_PROTOCOL_ISO_DEP)
...
BOOLEAN nfa_ce_api_reg_listen (tNFA_CE_MSG *p_ce_msg)
{
    ...
    {
        /* LIBNCI bugfix: LF_T3T_MAX limitation */
        /* formally, LF_T3T_MAX should be get by GetConfig, this is temp hack */
#ifndef SPZ_IMPL
        unsigned int num=0;
        if (GetNumValue(NAME_CE_LF_T3T_MAX_LIMIT, &num, sizeof(num)) && num)
#else
        unsigned int num=2;
#endif /* SPZ_IMPL */
        {
            ...
            if (num > T3T_MAX_SYSTEM_CODES)
                error_flag = TRUE;

            if (num > (NFA_CE_LISTEN_INFO_MAX-1))
                num = (NFA_CE_LISTEN_INFO_MAX-1);

            if (p_ce_msg->reg_listen.listen_type == NFA_CE_REG_TYPE_FELICA)
            {
                ...
                for (i=1; i<NFA_CE_LISTEN_INFO_MAX; i++)
                {
                    if ((p_ce_msg->reg_listen.listen_type == NFA_CE_REG_TYPE_FELICA)
                        ...
                        &&(p_cb->listen_info[i].flags & NFA_CE_LISTEN_INFO_FELICA))
                    {
                        count++;
                    }
                }
                if (count >= num || error_flag)
                {
                    /* Notify application */
    ...
    else
    {
        ...
        case NFA_CE_REG_TYPE_FELICA:
            ...
            /* tempworkaround: HCEF has priority to T3T UICC start */
            if ( (p_ce_msg->reg_listen.system_code & 0xf000) == 0x4000 && p_ce_msg->reg_listen.nfcid2[0] == 0x02
                 && p_ce_msg->reg_listen.nfcid2[1] == 0xfe)
            {
                p_cb->listen_info[listen_info_idx].flags |= NFA_CE_LISTEN_INFO_FELICA_HCEF;
            }
            /* tempworkaround: HCEF has priority to T3T UICC end*/
            p_cb->listen_info[listen_info_idx].p_conn_cback = p_ce_msg->reg_listen.p_conn_cback;
