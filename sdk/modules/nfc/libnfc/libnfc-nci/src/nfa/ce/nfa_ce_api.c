                                           ...
                                           tNFA_TECHNOLOGY_MASK tech_mask)
{
...
#if (NFCNCI_DEBUGFUNC == TRUE)
    {
        ...
        int len = property_get("nfc.debug.uicclisten_mask", valueStr, "");
        if( len > 0 )
        {
            unsigned int num;
                ...
                sscanf (valueStr, "%u", &num) == 1 )
            {
                ...
                tech_mask &= num ;
                NFA_TRACE_DEBUG2("%s: nfc.debug.uicclisten_mask=0x%X", __FUNCTION__, num);
            }
        }
    }
#endif
...
tNFA_STATUS NFA_CeSetIsoDepListenTech (tNFA_TECHNOLOGY_MASK tech_mask)
{
    ...
    {
        char valueStr [PROPERTY_VALUE_MAX] = {0};
        int len = property_get("nfc.debug.isodeplisten_mask", valueStr, "");
        if( len > 0 )
        {
            unsigned int num;
            if( sscanf (valueStr, "0x%x", &num) == 1 ||
                sscanf (valueStr, "%u", &num) == 1 )
            {
                num &= NFA_TECHNOLOGY_MASK_ALL;
                tech_mask &= num ;
                NFA_TRACE_DEBUG2("%s: nfc.debug.isodeplisten_mask=0x%X", __FUNCTION__, num);
            }
        }
    }
#endif
