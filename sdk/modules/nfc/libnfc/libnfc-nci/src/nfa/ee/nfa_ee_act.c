...
#if defined(NFA_EEGETLMRTSTATUS_INCLUDED) && (NFA_EEGETLMRTSTATUS_INCLUDED==1)
/*******************************************************************************
**
** Function         nfa_ee_tech_prot_lmrt_size
**
** Description      the total listen mode routing table size
**
** Returns          UINT16
**
*******************************************************************************/
static UINT16 nfa_ee_tech_prot_lmrt_size(void)
{
    int xx;
    UINT16 lmrt_size = 0;
    tNFA_EE_ECB          *p_cb;
    ...
    NFA_TRACE_DEBUG1 ("nfa_ee_tech_prot_lmrt_size size:%d", lmrt_size);
    return lmrt_size;
}

/*******************************************************************************
**
** Function         nfa_ee_api_lmrt_status
**
** Description      Reports the status in the Listen Mode Routing Table
**
** Returns          void
**
*******************************************************************************/
void nfa_ee_api_lmrt_status(tNFA_EE_MSG *p_data)
{
    tNFA_EE_CBACK_DATA  evt_data = {0};

    evt_data.ee_lmrt_status.total_len = NFC_GetLmrtSize();
    evt_data.ee_lmrt_status.tech_prot_use_len = nfa_ee_tech_prot_lmrt_size();

    NFA_TRACE_DEBUG2 ("nfa_ee_api_lmrt_status total_len=%d tech_prot_use_len=%d", evt_data.ee_lmrt_status.total_len, evt_data.ee_lmrt_status.tech_prot_use_len);

    nfa_ee_report_event (NULL, NFA_EE_LMRT_STATUS_EVT, &evt_data);
}
#endif /*defined(NFA_EEGETLMRTSTATUS_INCLUDED) && (NFA_EEGETLMRTSTATUS_INCLUDED==1)*/
...
tNFA_EE_ECB * nfa_ee_find_aid_offset(UINT8 aid_len, UINT8 *p_aid, int *p_offset, int *p_entry)
{
    ...
    for (yy = 0; yy < nfa_ee_cb.cur_ee + 1; yy++) /*LIBNCI bugfix: donot update p_ecb, add +1(NFA_EE_CB_4_DH) */
    {
...
#if defined(NFA_EEDISCONNECTFORCE_INCLUDED) && (NFA_EEDISCONNECTFORCE_INCLUDED==1)
    /* Force Send ConnClose */
    if (p_cb->conn_st != NFA_EE_CONN_ST_CONN &&
        (p_data->hdr.layer_specific & NFA_EE_API_DISCONNECT_FLAGS_MASK) == NFA_EE_API_DISCONNECT_FLAGS_CONNID)
    {
        UINT8 conn_id = (p_data->hdr.layer_specific & 0xFF);
        tNFC_STATUS status = NFC_ConnDummyCreate( conn_id );
...
void nfa_ee_nci_disc_ntf(tNFA_EE_MSG *p_data)
{
    ...
    if (p_cb)
    {
        ...
        {
            /* CXD224x workaround: enable restore to NFCEE interface if type is HCI_ACCESS */
            unsigned int num=0;
            BOOLEAN hci_restore_en = FALSE; /* original: HCI_ACCESS is virtual */
            if (GetNumValue(NAME_CXD224X_WORKAROUND, &num, sizeof(num)) && (num & NFC_CXD_WORKAROUND_EE_RESTORE_HCI))
            {
                hci_restore_en = TRUE;  /* HCI_ACCESS is real interface */
            }
            ...
            if ((p_cb->ee_interface[0] != NFC_NFCEE_INTERFACE_HCI_ACCESS && hci_restore_en == FALSE)
                || (p_cb->ee_interface[0] == NFC_NFCEE_INTERFACE_HCI_ACCESS && hci_restore_en == TRUE))
            {
...
void nfa_ee_nci_mode_set_rsp(tNFA_EE_MSG *p_data)
{
    ...
    {
        ;/* do not start timer after mode_set_rsp */
    }
    else
    {
    ...
    else
    {
        ...
        ee_active_rep_flag = (p_cb->ee_status == NFC_NFCEE_STATUS_ACTIVE);
        if (GetNumValue(NAME_CXD224X_WORKAROUND, &num, sizeof(num)) && (num & NFC_CXD_WORKAROUND_NO_REQ_EVT_EE_ACTIVE))
        {
            ee_active_rep_flag = FALSE; /* DO NOT SEND the discover request event when EE mode is changed from Inactive to Active.*/
        }
    ...
    if (p_cb)
    {
        ...
        case NFC_CONN_CREATE_CEVT:
            ...
            else
            {
                ...
                p_cb->p_ee_cback = NULL; /* LIBNCI bugfix: clear ee_cback if CONN_CREATE was failed */
            }
            if (p_cb->ecb_flags & NFA_EE_ECB_FLAGS_RESTORE)
            {
                p_cb->ecb_flags &= ~NFA_EE_ECB_FLAGS_RESTORE;
...
tNFA_STATUS nfa_ee_route_add_one_ecb(tNFA_EE_ECB *p_cb, int *p_max_len, BOOLEAN more, UINT8 *ps, int *p_cur_offset)
{
    ...
    /* LIBNCI bugfix: temporary workaround, move NFCDep entry to jni, because of lmrt size miss much
