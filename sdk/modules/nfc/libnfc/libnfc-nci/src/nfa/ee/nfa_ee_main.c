...
const tNFA_EE_SM_ACT nfa_ee_actions[] =
{
    ...
    nfa_ee_api_lmrt_status  ,   /* NFA_EE_API_LMRT_STATUS_EVT   */
...
#include <assert.h>
    nfa_ee_cb_ptr = (tNFA_EE_CB *)GKI_os_malloc(sizeof(tNFA_EE_CB));
    assert (nfa_ee_cb_ptr);
#endif
...
/*******************************************************************************
**
** Function         nfa_ee_destroy
**
** Description      Destroy NFA EE control block
**
** Returns          None
**
*******************************************************************************/
void nfa_ee_destroy (void)
{
#if NFA_DYNAMIC_MEMORY == TRUE
    if (nfa_ee_cb_ptr)
        GKI_os_free(nfa_ee_cb_ptr);
    nfa_ee_cb_ptr=NULL;
#endif
}
...
void nfa_ee_proc_nfcc_power_mode (UINT8 nfcc_power_mode)
{
    ...
    if (nfcc_power_mode == NFA_DM_PWR_MODE_FULL)
    {
        if (nfa_ee_max_ee_cfg)
        {
            ...
            unsigned int num=0;
            BOOLEAN hci_restore_en = FALSE; /* org: HCI_ACCESS is virtual */
            ...
            for (xx = 0; xx < NFA_EE_MAX_EE_SUPPORTED; xx++, p_cb++)
            {
                ...
                if ((p_cb->nfcee_id != NFA_EE_INVALID) && (restore_cond == TRUE) && (p_cb->ee_status  != NFA_EE_STATUS_REMOVED))
                {
                    proc_complete       = FALSE;
