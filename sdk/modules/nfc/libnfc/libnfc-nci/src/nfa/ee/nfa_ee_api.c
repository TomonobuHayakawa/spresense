...
/*******************************************************************************
**
** Function         NFA_EeGetLmrtStatus
**
** Description      This function is called to get the status of the
**                  Listen Mode Routing Table.
**                  The status is reported in NFA_LMRT_STATUS_EVT
**
** Returns          NFA_STATUS_OK if successfully initiated
**                  NFA_STATUS_FAILED otherwise
**
*******************************************************************************/
tNFA_STATUS NFA_EeGetLmrtStatus (void)
{
    tNFA_EE_API_LMRT_SIZE *p_msg;
    tNFA_STATUS status = NFA_STATUS_FAILED;

    NFA_TRACE_API0 ("NFA_EeGetLmrtStatus()");
    if ((p_msg = (tNFA_EE_API_LMRT_STATUS *) GKI_getbuf (sizeof(tNFA_EE_API_LMRT_STATUS))) != NULL)
    {
        p_msg->event    = NFA_EE_API_LMRT_STATUS_EVT;
        nfa_sys_sendmsg (p_msg);
        status = NFA_STATUS_OK;
    }
    return status;
...
/*******************************************************************************
**
** Function         NFA_EeDisconnectForce
**
** Description      Disconnect (dont case connection status) from an
**                  NFCEE interface. The result of this operation is reported
**                  with the NFA_EE_DISCONNECT_EVT.
**                  This function send CORE_CONN_CLOSE_CMD with conn_id.
**
** Returns          NFA_STATUS_OK if successfully initiated
**                  NFA_STATUS_FAILED otherwise
**                  NFA_STATUS_INVALID_PARAM If bad parameter
**
*******************************************************************************/
tNFA_STATUS NFA_EeDisconnectForce(tNFA_HANDLE ee_handle, UINT8 conn_id)
{
    tNFA_EE_API_DISCONNECT *p_msg;
    tNFA_STATUS status = NFA_STATUS_FAILED;
    UINT8       nfcee_id = (UINT8)(ee_handle & 0xFF);
    ...
    NFA_TRACE_API2 ("NFA_EeDisconnectForce(): handle:<0x%x> conn_id:%d", ee_handle, conn_id);
    p_cb = nfa_ee_find_ecb (nfcee_id);

    if ((p_cb == NULL) || (conn_id > NFA_EE_CONN_ID_MAX))
    {
        NFA_TRACE_ERROR0 ("NFA_EeDisconnectForce() Bad conn_id");
        status = NFA_STATUS_INVALID_PARAM;
    }
    else if ((p_msg = (tNFA_EE_API_DISCONNECT *) GKI_getbuf (sizeof(tNFA_EE_API_DISCONNECT))) != NULL)
    {
        p_msg->hdr.event        = NFA_EE_API_DISCONNECT_EVT;
        p_msg->hdr.layer_specific = NFA_EE_API_DISCONNECT_FLAGS_CONNID|conn_id;
        p_msg->nfcee_id         = nfcee_id;
        p_msg->p_cb             = p_cb;
