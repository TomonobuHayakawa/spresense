...
tNFA_STATUS nfa_hciu_send_get_param_cmd (UINT8 pipe, UINT8 index)
{
    ...
    if( pipe == NFA_HCI_ADMIN_PIPE )
        NFA_TRACE_DEBUG1 ("nfa_hciu_get_parm_cmd: ADMIN_PIPE index=%s",
                          index == NFA_HCI_SESSION_IDENTITY_INDEX ? "SESSION_IDENTITY" :
                          index == NFA_HCI_MAX_PIPE_INDEX ? "MAX_PIPE" :
                          index == NFA_HCI_WHITELIST_INDEX ? "WHITELIST" :
                          index == NFA_HCI_HOST_LIST_INDEX ? "HOST_LIST" : "UnKnown" );
...
tNFA_STATUS nfa_hciu_send_set_param_cmd (UINT8 pipe, UINT8 index, UINT8 length, UINT8 *p_data)
{
    ...
    if( pipe == NFA_HCI_ADMIN_PIPE )
        NFA_TRACE_DEBUG1 ("nfa_hciu_send_parm_cmd: ADMIN_PIPE index=%s",
                          index == NFA_HCI_SESSION_IDENTITY_INDEX ? "SESSION_IDENTITY" :
                          index == NFA_HCI_MAX_PIPE_INDEX ? "MAX_PIPE" :
                          index == NFA_HCI_WHITELIST_INDEX ? "WHITELIST" :
                          index == NFA_HCI_HOST_LIST_INDEX ? "HOST_LIST" : "UnKnown" );
...
char *nfa_hciu_get_event_name (UINT16 event)
{
    ...
    case NFA_HCI_API_ADD_STATIC_PIPE_EVT:     return ("API_ADD_STATIC_PIPE");  /* missing */
    ...
    case NFA_HCI_API_DISCONNECT_EVT:          return ("API_DISCONNECT_EVT"); /* add NFA_HciDisconnect*/
    case NFA_HCI_API_DELETEGATEANDPIPE_EVT:   return ("API_DELETEGATEANDPIPE_EVT"); /* add NFA_HciDeleteGateAndPipe*/
    ...
    case NFA_HCI_CHECK_TIMEOUT_EVT:           return ("CHECK_TIMEOUT_EVT"); /* WAIT_NETWK_ENABLE_PROP */
...
char *nfa_hciu_get_state_name (UINT8 state)
{
    ...
    case NFA_HCI_STATE_WAIT_NETWK_ENABLE_PROP:   return ("WAIT_NETWK_ENABLE_PROP");
    case NFA_HCI_STATE_WAIT_DISCONNECT:      return ("WAIT_DISCONNECT");
    case NFA_HCI_STATE_APP_DELETEGATEANDPIPE: return ("APP_DELETEGATEANDPIPE");
    default:
        return ("UNKNOWN");
    }
}
