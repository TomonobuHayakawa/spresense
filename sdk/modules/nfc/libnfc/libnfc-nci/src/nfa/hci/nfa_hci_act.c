...
void nfa_hci_check_api_requests (void)
{
    ...
    for ( ; ; )
    {
        ...
        case NFA_HCI_API_DELETEGATEANDPIPE_EVT:
            nfa_hci_api_deletegateandpipe (p_evt_data);
            break;

        case NFA_HCI_API_DISCONNECT_EVT:
...
void nfa_hci_handle_admin_gate_cmd (UINT8 *p_data)
{
    ...
    case NFA_HCI_ADM_NOTIFY_ALL_PIPE_CLEARED:
        ...
        else
        {
            ...
            /* LIBNCI workaround: if receive ALL_PIPE_CLEARED from UICC, change netwk_init flag */
            NFA_TRACE_DEBUG0 ("nfa_hci_handle_admin_gate_cmd - ALL_PIPE_CLEARED received netwk_init");
...
#if (defined (NFA_HCIDISCONNECT_INCLUDED) && (NFA_HCIDISCONNECT_INCLUDED==TRUE))
            else if (nfa_hci_cb.hci_state == NFA_HCI_STATE_APP_DELETEGATEANDPIPE)
                nfa_hci_api_deletegateandpipe (NULL);
#endif /*(defined (NFA_HCIDISCONNECT_INCLUDED) && (NFA_HCIDISCONNECT_INCLUDED==TRUE))*/
            break;
...
#define NFA_HCI_UIM_POLLING_INTERVAL_MAX 2000
/*******************************************************************************
**
    ...
    if (p_pipe == NULL)
    {
            ...
            && pipe_id == 0x09)
        {
                ...
                && nfa_hci_cb.hci_state == NFA_HCI_STATE_WAIT_NETWK_ENABLE_PROP)
            {
                /* retry */
                int num=0;
                if( GetNumValue(NAME_NFA_HCI_UIM_POLLING_INTERVAL, &num, sizeof(num)) && num > 0 )
                {
                    if( num > NFA_HCI_UIM_POLLING_INTERVAL_MAX )
                        num = NFA_HCI_UIM_POLLING_INTERVAL_MAX;
                    GKI_delay(num);
                }
            ...
            {
                NFA_TRACE_DEBUG0 ("nfa_hci_handle_dyn_pipe_pkt network enable(prop) done");
...
/*******************************************************************************
**
** Function         nfa_hci_api_disconnect
**
** Description      action function to ConnClose HCI interface
**
** Returns          None
**
*******************************************************************************/
void nfa_hci_api_disconnect (tNFA_HCI_EVENT_DATA *p_evt_data)
{
    ...
    {
        NFA_TRACE_DEBUG1 ("nfa_hci_api_disconnect() connid=0x%x", nfa_hci_cb.conn_id);
        status = NFC_ConnClose (nfa_hci_cb.conn_id);
        nfa_hci_cb.hci_state = NFA_HCI_STATE_WAIT_DISCONNECT;
    }
    else
    {
        tNFA_HCI_EVT_DATA   evt_data;
        status = evt_data.hci_disconnect.status = p_evt_data->disconnect.status;
        /* notify NFA_HCI_DISCONNECT_EVT to the all application */
        nfa_hciu_send_to_all_apps (NFA_HCI_DISCONNECT_EVT, &evt_data);
    }
    NFA_TRACE_DEBUG1 ("nfa_hci_api_disconnect() status=0x%x", status);
}

/*******************************************************************************
**
** Function         nfa_hci_api_deletegateandpipe
**
** Description      action function to deletegateandpipe the given application
**
** Returns          None
**
*******************************************************************************/
/* note: this function should be merged to nfa_hci_api_deregiter() */
void nfa_hci_api_deletegateandpipe (tNFA_HCI_EVENT_DATA *p_evt_data)
{
    tNFA_HCI_EVT_DATA   evt_data;
    tNFA_HCI_CBACK      *p_cback = NULL;
    int                 xx;
    ...
    if (p_evt_data != NULL)
    {
        ...
        {
            if (  (nfa_hci_cb.cfg.reg_app_names[xx][0] != 0)
                  && !strncmp (p_evt_data->app_info.app_name, &nfa_hci_cb.cfg.reg_app_names[xx][0], strnlen (p_evt_data->app_info.app_name, NFA_MAX_HCI_APP_NAME_LEN)) )
            {
                NFA_TRACE_EVENT2 ("nfa_hci_api_deletegateandpipe (%s) inx: %u", p_evt_data->app_info.app_name, xx);
                break;
            }
        }
        ...
        if (xx == NFA_HCI_MAX_APP_CB)
        {
            NFA_TRACE_WARNING1 ("nfa_hci_api_deletegateandpipe () Unknown app: %s", p_evt_data->app_info.app_name);
            return;
        }
        nfa_hci_cb.app_in_use = (tNFA_HANDLE) (xx | NFA_HANDLE_GROUP_HCI);
        p_cback               = nfa_hci_cb.p_app_cback[xx];
    ...
    if (nfa_hciu_find_pipe_by_owner (nfa_hci_cb.app_in_use) == NULL)
    {
        ...
        evt_data.hci_deletegateandpipe.status = NFC_STATUS_OK;

        if (nfa_hci_cb.hci_state == NFA_HCI_STATE_APP_DELETEGATEANDPIPE)
            nfa_hci_cb.hci_state = NFA_HCI_STATE_IDLE;

        /* notify NFA_HCI_DELETEGATEANDPIPE_EVT to the application */
        if (p_cback)
            p_cback (NFA_HCI_DELETEGATEANDPIPE_EVT, &evt_data);
    }
    else if ((p_pipe = nfa_hciu_find_active_pipe_by_owner (nfa_hci_cb.app_in_use)) == NULL)
    {
        /* No pipes, release all gates owned by this app */
        ...
        evt_data.hci_deletegateandpipe.status = NFC_STATUS_FAILED;
    ...
    {
        /* Delete all active pipes created for the application before cback
        **/
        nfa_hci_cb.hci_state = NFA_HCI_STATE_APP_DELETEGATEANDPIPE;
