...
#else
#include <stdlib.h>
tNFA_HCI_CB *nfa_hci_cb_ptr=NULL;
#endif
...
#ifndef NFA_HCI_HCI_UIM_POLLING_PREWAIT_MAX
#define NFA_HCI_HCI_UIM_POLLING_PREWAIT_MAX 2000
#endif

#ifndef NFA_HCI_INTF_ACTIVE_FAIL_MAX
#define NFA_HCI_INTF_ACTIVE_FAIL_MAX 5
#endif
...
void nfa_hci_ee_info_cback (tNFA_EE_DISC_STS status)
{
    ...
    unsigned int num = 0;
    NFA_TRACE_DEBUG2 ("nfa_hci_ee_info_cback (): %d %s", status, nfa_hciu_get_state_name(nfa_hci_cb.hci_state));
    ...
    case NFA_EE_DISC_STS_OFF:
        ...
        if (  (nfa_hci_cb.hci_state == NFA_HCI_STATE_WAIT_NETWK_ENABLE)
            ||(nfa_hci_cb.hci_state == NFA_HCI_STATE_WAIT_NETWK_ENABLE_PROP)
    ...
    case NFA_EE_DISC_STS_REQ:
        ...
        if( GetNumValue(NAME_CXD224X_WORKAROUND, &num, sizeof(num)) && (num & NFC_CXD_WORKAROUND_EE_REQ_NTF) )
        {
            /* Ignore extra Discovery Ntf */
        ...
        if (nfa_hci_cb.ee_disable_disc)
        {
                ...
                ||(nfa_hci_cb.hci_state == NFA_HCI_STATE_RESTORE_NETWK_ENABLE)  )
            {
                ...
                if (nfa_hci_cb.num_ee_dis_req_ntf == (nfa_hci_cb.num_nfcee - 1))
                {
                    ...
                    nfa_sys_stop_timer (&nfa_hci_cb.timer);
                    if( nfa_hci_cb.hci_state == NFA_HCI_STATE_WAIT_NETWK_ENABLE_PROP )
                    {
                        int num=0;
                        if (GetNumValue(NAME_NFA_HCI_UIM_POLLING_PREWAIT, &num, sizeof(num)) && num > 0)
                        {
                            if( num > NFA_HCI_HCI_UIM_POLLING_PREWAIT_MAX )
                                num = NFA_HCI_HCI_UIM_POLLING_PREWAIT_MAX;
                            nfa_sys_start_timer (&nfa_hci_cb.timer, NFA_HCI_RSP_TIMEOUT_EVT, num);
                        }
                        else
                        {
                            /* Start GET session ID form prop gate(fixed pipe) for checking UICC bootup complete */
                            /* timeout to check the change of the session id */
                            nfa_sys_start_timer (&nfa_hci_cb.pr_timer, NFA_HCI_CHECK_TIMEOUT_EVT, NFA_HCI_UIM_POLLING_NORMAL_TIMEOUT);
                            nfa_hciu_send_get_param_cmd ( 0x09, NFA_HCI_SESSION_IDENTITY_INDEX);
                        }
                    }
                    else
                    {
...
/* CXD224x workaround: Delayed NTF packts(EE_DIC_REQ_NTF) start */
/*  also must build with NFC_HAL_DELAYED_NTF_INCLUDED==TRUE */
static void nfa_hci_send_halcc(UINT8 cmd, UINT8 parm)
{
    tNFA_DM_API_SEND_VSC *p_msg;
    UINT16  size = sizeof(tNFA_DM_API_SEND_VSC) + 1; /*use VSC struct*/
    if ((p_msg = (tNFA_DM_API_SEND_VSC *) GKI_getbuf (size)) != NULL)
    {
        BT_HDR  *p_cmd;

        p_msg->hdr.event        = NFA_DM_API_HALCC_EVT;
        p_msg->oid              = cmd; //NFC_HAL_REQ_DELAYED_PKTS
        p_msg->p_cback          = NULL;
        p_msg->cmd_params_len   = 1;
        p_msg->p_cmd_params     = (UINT8 *)(p_msg + 1);
        *(p_msg->p_cmd_params)  = parm;
        p_cmd = (BT_HDR *)p_msg;

        p_cmd->offset   = sizeof (tNFA_DM_API_SEND_VSC) - BT_HDR_SIZE;
        p_cmd->len      = p_msg->cmd_params_len;
        NFC_SendHalControlCommand (p_msg->oid, p_cmd, p_msg->p_cback);
    }
}
/* CXD224x workaround: Delayed NTF packts(EE_DIC_REQ_NTF) end */
...
#if NFA_DYNAMIC_MEMORY == TRUE
#include <assert.h>
    nfa_hci_cb_ptr = (tNFA_HCI_CB *)GKI_os_malloc( sizeof(tNFA_HCI_CB) );
    assert (nfa_hci_cb_ptr);
#endif
...
/*******************************************************************************
**
** Function         nfa_hci_destroy
**
** Description      Destroy NFA HCI
**
** Returns          None
**
*******************************************************************************/
void nfa_hci_destroy (void)
{
#if NFA_DYNAMIC_MEMORY == TRUE
    if (nfa_hci_cb_ptr)
        GKI_os_free (nfa_hci_cb_ptr);
    nfa_hci_cb_ptr = NULL;
#endif
}
...
{
    NFA_TRACE_EVENT1 ("nfa_hci_dh_startup_complete (): hci_state: %u", nfa_hci_cb.hci_state);
    if (nfa_hci_cb.w4_hci_netwk_init)
    {
        if (nfa_hci_cb.hci_state == NFA_HCI_STATE_STARTUP)
        {
            /* CXD224x workaround: support variable modeset sequence */
            unsigned int num=0;
            /* Send ModeSet(UICC poweron) after HCI Network is completed */
            if( GetNumValue(NAME_NFA_HCI_DISCOVERY_SEQ, &num, sizeof(num))
                && ( (num==NFC_NFA_HCI_DISCOVERY_SEQ_DELAYEDMODESET1)
                     ||(num==NFC_NFA_HCI_DISCOVERY_SEQ_DELAYEDMODESET1_UIM)) )
            {
                tNFA_EE_INFO    ee_info[NFA_EE_MAX_EE_SUPPORTED];
                UINT8           num_nfcee = NFA_EE_MAX_EE_SUPPORTED;
                UINT8           target_handle;
                UINT8           count = 0;
                NFA_EeGetInfo (&num_nfcee, ee_info);
                ...
                if( num==NFC_NFA_HCI_DISCOVERY_SEQ_DELAYEDMODESET1_UIM )
                {
                    /* CXD224x workaround: checking UIM Management gate(fixed pipe,proprietay) */
                    nfa_hci_cb.hci_state = NFA_HCI_STATE_WAIT_NETWK_ENABLE_PROP;
                }
                else
                {
...
#if (defined (NFA_HALCC_INCLUDED) && (NFA_HALCC_INCLUDED==TRUE))
                /* CXD224x workaround: Delayed NTF packts(EE_DIC_REQ_NTF) start */
                /* HAL must be builded with NFC_HAL_DELAYED_NTF_INCLUDED==TRUE */
                if (GetNumValue(NAME_CXD224X_WORKAROUND, &num, sizeof(num)) && (num & NFC_CXD_WORKAROUND_HAL_DELAYED_PKTS))
                {
                    /* send request to drain saved pkts to HAL */
                    nfa_hci_send_halcc(NFC_HAL_REQ_DELAYED_PKTS, NFC_HAL_REQ_DELAYED_PKTS_DRAIN);
                }
                /* CXD224x workaround: Delayed NTF packts(EE_DIC_REQ_NTF) end */
#endif /*(defined (NFA_HALCC_INCLUDED) && (NFA_HALCC_INCLUDED==TRUE))*/
            }
...
void nfa_hci_startup_complete (tNFA_STATUS status)
{
    ...
    nfa_sys_stop_timer (&nfa_hci_cb.timer);
    nfa_sys_stop_timer (&nfa_hci_cb.pr_timer);

    /* CXD224x workaround: notify hci_nv_write to jni start */
    if (status == NFA_STATUS_OK && nfa_hci_cb.nv_write_needed)
    {
        nfa_sys_cback_notify_hci_nv_write_event ();
    }
    ...
    if (nfa_hci_cb.nv_read_cmplt && nfa_hci_cb.ee_disc_cmplt && (nfa_hci_cb.conn_id == 0))
    {
        ...
        while ((count < num_nfcee) && (!found))
        {
            ...
            if(ee_info[count].ee_interface[0] == NFA_EE_INTERFACE_HCI_ACCESS)
            {
                ...
                if( !GetNumValue(NAME_NFA_HCI_DISCOVERY_SEQ, &num, sizeof(num)) || (num==NFC_NFA_HCI_DISCOVERY_SEQ_MODESET_1ST) )
                {
                    if (ee_info[count].ee_status == NFA_EE_STATUS_INACTIVE)
                    {
                        NFC_NfceeModeSet (target_handle, NFC_MODE_ACTIVATE);
                ...
                if ((status = NFC_ConnCreate (NCI_DEST_TYPE_NFCEE, target_handle, NFA_EE_INTERFACE_HCI_ACCESS, nfa_hci_conn_cback)) == NFA_STATUS_OK)
                {
                    ...
                    if (GetNumValue(NAME_CXD224X_WORKAROUND, &num, sizeof(num)) && (num & NFC_CXD_WORKAROUND_HAL_DELAYED_PKTS))
                    {
                        /* send request to STOP saveing pkt to HAL */
                        nfa_hci_send_halcc(NFC_HAL_REQ_DELAYED_PKTS, NFC_HAL_REQ_DELAYED_PKTS_STOP);
                    }
#endif /*(defined (NFA_HALCC_INCLUDED) && (NFA_HALCC_INCLUDED==TRUE))*/
                }
                else
...
static void nfa_hci_conn_cback (UINT8 conn_id, tNFC_CONN_EVT event, tNFC_CONN *p_data)
{
    ...
    else if (event == NFC_CONN_CLOSE_CEVT)
    {
        ...
        /* CXD224x workaround: notify hci_nv_write to jni start */
        if (nfa_hci_cb.nv_write_needed)
        {
            nfa_sys_cback_notify_hci_nv_write_event ();
        }
        /* CXD224x workaround: notify hci_nv_write to jni end */

#if (defined (NFA_HCIDISCONNECT_INCLUDED) && (NFA_HCIDISCONNECT_INCLUDED==TRUE))
        if (nfa_hci_cb.hci_state == NFA_HCI_STATE_WAIT_DISCONNECT)
        {
            tNFA_HCI_EVENT_DATA p_evt_data;
            p_evt_data.disconnect.status = p_data->status;
            nfa_hci_api_disconnect(&p_evt_data);
            nfa_hci_cb.hci_state = NFA_HCI_STATE_IDLE;
        }
        else
#endif /*(defined (NFA_HCIDISCONNECT_INCLUDED) && (NFA_HCIDISCONNECT_INCLUDED==TRUE))*/
        {
            nfa_hci_cb.hci_state = NFA_HCI_STATE_DISABLED;
            /* deregister message handler on NFA SYS */
            nfa_sys_deregister (NFA_ID_HCI);
             ...
             (p_data->status == NFA_STATUS_EE_INTF_ACTIVE_FAIL))
    {
        if (nfa_hci_cb.hci_state == NFA_HCI_STATE_WAIT_NETWK_ENABLE_PROP)
        {
            /* when recv NFA_STATUS_EE_INTF_ACTIVE_FAIL, NFCC cannot recv EE DISC REQ Ntf(s).
             * Stop timer for early termination */
            nfa_sys_stop_timer (&nfa_hci_cb.timer);

            /* when recv NFA_STATUS_EE_INTF_ACTIVE_FAIL, UICC is probably not present */
            /* use short timeout value */
            nfa_sys_start_timer (&nfa_hci_cb.pr_timer, NFA_HCI_CHECK_TIMEOUT_EVT, NFA_HCI_UIM_POLLING_ABNORMAL_TIMEOUT);

            /* Start GET session ID form prop gate(fixed pipe) for checking bootup complete */
            nfa_hciu_send_get_param_cmd ( 0x09, NFA_HCI_SESSION_IDENTITY_INDEX);
        }
        else if (nfa_hci_cb.hci_state == NFA_HCI_STATE_IDLE)
        {
            nfa_hci_cb.intf_fail_count++;
            NFA_TRACE_DEBUG1 ("nfa_hci_conn_cback () intf_active_fail_count=%d", nfa_hci_cb.intf_fail_count);
            if (nfa_hci_cb.intf_fail_count >= NFA_HCI_INTF_ACTIVE_FAIL_MAX)
            {
                unsigned int num = 0;
                /* Disable NFCEE connected via NFA_EE_INTERFACE_HCI_ACCESS when recv multiple NFA_STATUS_EE_INTF_ACTIVE_FAIL */
                if (GetNumValue(NAME_CXD224X_WORKAROUND, &num, sizeof(num)) && (num & NFC_CXD_WORKAROUND_HCI_DI_INTF_FAIL))
                {
                    tNFA_EE_INFO    ee_info[2];
                    UINT8           num_nfcee = 2;
                    UINT8           target_handle;
                    ...
                    while ((count < num_nfcee) && (!found))
                    {
                        ...
                        {
                            found = TRUE;
                            if (ee_info[count].ee_status == NFA_EE_STATUS_ACTIVE)
                            {
                                NFC_NfceeModeSet (target_handle, NFC_MODE_DEACTIVATE);
                            }
                        }
                        count++;
                    }
                }
                nfa_sys_cback_notify_hci_act_fail_event (); /* CXD224x workaround: notify hci_act_fail to jni */
            }
        }
    }
...
void nfa_hci_rsp_timeout (tNFA_HCI_EVENT_DATA *p_evt_data)
{
    ...
    case NFA_HCI_STATE_WAIT_NETWK_ENABLE_PROP:
        if( ((BT_HDR *)p_evt_data)->event ==  NFA_HCI_RSP_TIMEOUT_EVT )
        {
            /* Start GET session ID form prop gate(fixed pipe) for checking bootup complete */
            nfa_sys_start_timer (&nfa_hci_cb.pr_timer, NFA_HCI_CHECK_TIMEOUT_EVT, NFA_HCI_UIM_POLLING_NORMAL_TIMEOUT);
            nfa_hciu_send_get_param_cmd ( 0x09, NFA_HCI_SESSION_IDENTITY_INDEX);
...
static BOOLEAN nfa_hci_evt_hdlr (BT_HDR *p_msg)
{
    ...
    else
    {
        ...
        case NFA_HCI_CHECK_TIMEOUT_EVT:
    ...
    if ((nfa_hci_cb.hci_state == NFA_HCI_STATE_IDLE) && (nfa_hci_cb.nv_write_needed))
    {
        ...
        NFA_TRACE_DEBUG5 ("nfa_hci_evt_hdlr () sz=%u dyn_gates[0]={id:0x%02x,own:0x%04x,pipe_inx_mask:0x%08x} dyn_pipes[0]=0x%x",
                          sizeof(nfa_hci_cb.cfg.reg_app_names),
                          nfa_hci_cb.cfg.dyn_gates[0].gate_id,
                          nfa_hci_cb.cfg.dyn_gates[0].gate_owner,
                          nfa_hci_cb.cfg.dyn_gates[0].pipe_inx_mask,
                          nfa_hci_cb.cfg.dyn_pipes[0].pipe_id );
