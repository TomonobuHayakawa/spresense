...
/*******************************************************************************
**
** Function         NFA_HciDisconnect
**
** Description      Disconnect (if a connection is currently opened) from an
**                  HCI interface. The result of this operation is reported
**                  with the NFA_HCI_DISCONNECT_EVT (if the app is still registered).
**
** Returns          NFA_STATUS_OK if successfully initiated
**                  NFA_STATUS_FAILED otherwise
**
*******************************************************************************/
NFC_API extern tNFA_STATUS NFA_HciDisconnect (tNFA_HANDLE hci_handle)
{
    tNFA_HCI_API_DISCONNECT *p_msg;
    tNFA_STATUS status = NFA_STATUS_FAILED;

    if ((NFA_HANDLE_GROUP_MASK & hci_handle) != NFA_HANDLE_GROUP_HCI)
    {
        NFA_TRACE_API1 ("NFA_HciDisconnect (): Invalid hci_handle:0x%04x", hci_handle);
        return (NFA_STATUS_FAILED);
    }

    NFA_TRACE_API1 ("NFA_HciDisconnect(): handle:<0x%x>", hci_handle);

    if ((nfa_hci_cb.hci_state != NFA_HCI_STATE_DISABLED)
        &&((p_msg = (tNFA_HCI_API_DISCONNECT *) GKI_getbuf (sizeof(tNFA_HCI_API_DISCONNECT))) != NULL) )
    {
        p_msg->hdr.event        = NFA_HCI_API_DISCONNECT_EVT;
        p_msg->hci_handle       = hci_handle;
...
/*******************************************************************************
**
** Function         NFA_HciDeleteGateAndPipe
**
** Description      This function is called to delete gate and pipe owned by the
**                  application. The app will be notified by NFA_HCI_DELETEGATEANDPIPE_EVT
**                  after deleting all the pipes owned by the app and deallocating
**                  all the gates allocated to the application or if an error occurs.
**
** Returns          NFA_STATUS_OK if successfully initiated
**                  NFA_STATUS_FAILED otherwise
**
*******************************************************************************/
tNFA_STATUS NFA_HciDeleteGateAndPipe (char *p_app_name)
{
    tNFA_HCI_API_REGISTER_APP *p_msg;
    int                         xx;
    UINT8                       app_name_len;
    ...
    if (p_app_name == NULL)
    {
        NFA_TRACE_API0 ("NFA_HciDeleteGateAndPipe (): Invalid Application");
        return (NFA_STATUS_FAILED);
    }

    NFA_TRACE_API1 ("NFA_HciDeleteGateAndPipe (): Application Name: %s", p_app_name);
    app_name_len = (UINT8) strnlen (p_app_name, NFA_MAX_HCI_APP_NAME_LEN);
    ...
    if (xx == NFA_HCI_MAX_APP_CB)
    {
        NFA_TRACE_ERROR1 ("NFA_HciDeleteGateAndPipe (): Application Name: %s  NOT FOUND", p_app_name);
        return (NFA_STATUS_FAILED);
    }

    /* DeleteGateAndPipe the application with HCI */
    if (  (nfa_hci_cb.hci_state != NFA_HCI_STATE_DISABLED)
        &&((p_msg = (tNFA_HCI_API_REGISTER_APP *) GKI_getbuf (sizeof (tNFA_HCI_API_REGISTER_APP))) != NULL) )
    {
        p_msg->hdr.event  = NFA_HCI_API_DELETEGATEANDPIPE_EVT;
