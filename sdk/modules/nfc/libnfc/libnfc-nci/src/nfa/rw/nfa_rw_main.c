...
#include "nfc_int.h"
#include "rw_int.h" /* LIBNCI bugfix: FIX_MEMORY_LEAK_0001 */
...
#include <assert.h>
    nfa_rw_cb_ptr = (tNFA_RW_CB *)GKI_os_malloc( sizeof(tNFA_RW_CB) );
    assert (nfa_rw_cb_ptr);
#endif
...
/*******************************************************************************
**
** Function         nfa_rw_destroy
**
** Description      Destroy NFA RW
**
** Returns          None
**
*******************************************************************************/
void nfa_rw_destroy (void)
{
#if NFA_DYNAMIC_MEMORY == TRUE
    if (nfa_rw_cb_ptr)
        GKI_os_free (nfa_rw_cb_ptr);
    nfa_rw_cb_ptr = NULL;
#endif
}
...
{
    /* LIBNCI bugfix: FIX_MEMORY_LEAK_0001 begin */
    tRW_T1T_CB  *p_t1t;
    tRW_T2T_CB  *p_t2t;
    tRW_T3T_CB  *p_t3t;
    tRW_T4T_CB  *p_t4t;
    ...
    NFA_TRACE_DEBUG1 ("nfa_rw_sys_disable() rw_cb.tcb_type=%d", rw_cb.tcb_type);
    ...
    case RW_CB_TYPE_T3T:
        p_t3t  = &rw_cb.tcb.t3t;
        nfc_stop_quick_timer (&p_t3t->timer);
        nfc_stop_quick_timer (&p_t3t->poll_timer);
    ...
    /* LIBNCI bugfix: FIX_MEMORY_LEAK_0001 end */
