...
static void nfa_rw_handle_t2t_evt (tRW_EVENT event, tRW_DATA *p_rw_data)
{
    ...
    {
        /* CXD224x workaround;
         * when Nack message was received, t2t->state go to IDLE state.
         * do not change halt_event even if receive Raw frame DATA message.
         */
        unsigned long num=0;
        if( !RW_T2tIsActiveState() && (event == RW_T2T_RAW_FRAME_EVT) &&
            (nfa_rw_cb.halt_event != RW_T2T_MAX_EVT) &&
            GetNumValue(NAME_CXD224X_WORKAROUND, &num, sizeof(num)) &&
            (num & NFC_CXD_WORKAROUND_T2TNACK_IDLE) )
        {
            NFA_TRACE_DEBUG2("nfa_rw_handle_t2t_evt; rw_flags=0x%x halt_evnt=0x%x :if recv RAW_FRAME under IDLE or NotActive state, Do not overwrite halt_event",
                             nfa_rw_cb.flags, nfa_rw_cb.halt_event);
        }
        else
            nfa_rw_cb.halt_event     = RW_T2T_MAX_EVT;
    }
...
static void nfa_rw_handle_t3t_evt (tRW_EVENT event, tRW_DATA *p_rw_data)
{
    ...
    case RW_T3T_POLL_EVT:                       /* t3t poll completed */
        /* Command complete - perform cleanup */
        nfa_rw_command_complete();
        ...
        /* DTA workaronud */
        if( event == RW_T3T_POLL_EVT )
            nfa_dm_act_conn_cback_notify(NFA_POLL_CPLT_EVT, &conn_evt_data);
        break;
...
void nfa_rw_presence_check (tNFA_RW_MSG *p_data)
{
    ...
    {
        /* CXD224x workaround: PresenceCheck T4T(FRMAE_RF_INTERFACE) start */
        if (nfa_rw_cb.intf_type == NFC_INTERFACE_FRAME)
        {   /* T4T FRAME_RF_INTERFACE */
            sendpkt_enable = TRUE;
        }
        /* CXD224x workaround: Send Deselect T4T(FRMAE_RF_INTERFACE) end */
    ...
    if (unsupported)
    {
        ...
        {
            /* CXD224x workaround: PresenceCheck T4T(FRMAE_RF_INTERFACE) start */
            /* Send Deselect command during PresenceCheck when connected to NfcA/NfcB */
            unsigned int num=0;
            if ((sendpkt_enable == TRUE) &&
                GetNumValue(NAME_CXD224X_WORKAROUND, &num, sizeof(num)) && (num & NFC_CXD_WORKAROUND_T4TPRESENCECHK))
            {
                BT_HDR *p_msg;
                UINT16 size = BT_HDR_SIZE + NCI_MSG_OFFSET_SIZE + NCI_DATA_HDR_SIZE + 2;
                if ((p_msg = (BT_HDR *)GKI_getbuf (size)) != NULL)
                {
                    ...
                    p[0] = 0xc2;  /* HALT:0x50, DESELCT:0xc2 */
                    p[1] = 0x00;
                    NFC_SendData (NFC_RF_CONN_ID, p_msg);
                }
                /* ugly delay: prevent PresenceCheck lost when activation fail,
                               add interval before Deactivate(sleep) for yeild to hal_thread */
                GKI_delay(10);
            }
            /* CXD224x workaround: Send Deselect T4T(FRMAE_RF_INTERFACE) end */
...
/*******************************************************************************
**
** Function         nfa_rw_t3t_poll
**
** Description      Handler for T3t_Poll API(manual issue polling command for dta)
**
** Returns          TRUE (message buffer to be freed by caller)
**
*******************************************************************************/
static BOOLEAN nfa_rw_t3t_poll (tNFA_RW_MSG *p_data)
{
    tNFA_RW_OP_PARAMS_T3T_POLL *p_t3t_poll = (tNFA_RW_OP_PARAMS_T3T_POLL *)&(p_data->op_req.params.t3t_poll);

    if (RW_T3tPoll (p_t3t_poll->system_code, p_t3t_poll->rc, p_t3t_poll->tsn) != NFC_STATUS_OK)
        nfa_rw_error_cleanup (NFA_POLL_CPLT_EVT);
...
static void nfa_rw_raw_mode_data_cback (UINT8 conn_id, tNFC_CONN_EVT event, tNFC_CONN *p_data)
{
    ...
    else if (event == NFC_DATA_CEVT) /* LIBNCI bugfix: FIX_MEMORY_LEAK begin */
    {
        p_msg = (BT_HDR *)p_data->data.p_data;
        if (p_msg)
        {
            NFA_TRACE_ERROR0 ("nfa_rw_raw_mode_data_cback (): error packet freebuf");
            GKI_freebuf (p_msg);
        }
    } /* LIBNCI bugfix: FIX_MEMORY_LEAK end */
}
...
BOOLEAN nfa_rw_activate_ntf(tNFA_RW_MSG *p_data)
{
    ...
    {
        /* DTA workaround : if DTA mode, skip issue RF_T3T_POLLING_CMD*/
        int num=0;
        if( GetNumValue(NAME_DTA_MODE, &num, sizeof(num)) && num == NFC_NFA_DTA_MODE_TRUE )
        {
            NFA_TRACE_DEBUG0 ("nfa_rw_activate_ntf(): skip T3T_GET_SYSTEM_CODES " );
            tNFA_TAG_PARAMS tag_params;
            /* Command complete - perform cleanup, notify app */
            nfa_rw_command_complete();
            tag_params.t3t.num_system_codes = 0;
        ...
        {
        /* Issue command to get Felica system codes s*/
...
BOOLEAN nfa_rw_handle_op_req (tNFA_RW_MSG *p_data)
{
    ...
    case NFA_RW_OP_T3T_GET_SYSTEM_CODES:
        ...
        /* DTA workaround */
    case NFA_RW_OP_T3T_POLL:
        nfa_rw_t3t_poll(p_data);
        break;
