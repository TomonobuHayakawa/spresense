...
/*******************************************************************************
**
** Function         NFA_RwT3tPoll
**
** Description:
**      Send an RF_T3T_POLLING_CMD command to the activated Type 3 tag.
**
**      When the poll operation has completed (or if an error occurs), the
**      app will be notified with NFA_POLL_CPLT_EVT.
**
** Returns:
**      NFA_STATUS_OK if successfully initiated
**      NFA_STATUS_FAILED otherwise
**
*******************************************************************************/
tNFA_STATUS NFA_RwT3tPoll (UINT16 system_code, UINT8 rc, UINT8 tsn)
{
    tNFA_RW_OPERATION *p_msg;

    NFA_TRACE_API1 ("NFA_RwT3tPoll (): system_code: %i", system_code);

    /* Validate parameters */
    ;

    if ((p_msg = (tNFA_RW_OPERATION *) GKI_getbuf ((UINT16) (sizeof (tNFA_RW_OPERATION) + 4))) != NULL)
    {
        /* Fill in tNFA_RW_OPERATION struct */
        p_msg->hdr.event = NFA_RW_OP_REQUEST_EVT;
        p_msg->op        = NFA_RW_OP_T3T_POLL;

        p_msg->params.t3t_poll.system_code   = system_code;
        p_msg->params.t3t_poll.rc            = rc;
        p_msg->params.t3t_poll.tsn           = tsn;
