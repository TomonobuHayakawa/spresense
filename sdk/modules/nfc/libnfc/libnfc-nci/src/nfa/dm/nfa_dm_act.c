...
/*******************************************************************************
**
** Function         nfa_dm_notify_nv_write_cback
**
** Description      Processing initialization complete event from sub-modules
**
** Returns          None
**
*******************************************************************************/
static void nfa_dm_notify_nv_write_cback (void)
{
    tNFA_DM_CBACK_DATA dm_cback_data;

    /* All subsystem are initialized */
    dm_cback_data.status = NFA_STATUS_OK;
    (*nfa_dm_cb.p_dm_cback) (NFA_DM_NOTIFY_NV_WRITE_EVT, &dm_cback_data);
}
/* CXD224x workaround: notify hci_nv_write to jni end */
/* CXD224x workaround: notify hci_act_fail to jni start */
/*******************************************************************************
**
** Function         nfa_dm_notify_act_fail_cback
**
** Description      Processing initialization complete event from sub-modules
**
** Returns          None
**
*******************************************************************************/
static void nfa_dm_notify_act_fail_cback (void)
{
    tNFA_DM_CBACK_DATA dm_cback_data;

    /* All subsystem are initialized */
    dm_cback_data.status = NFA_STATUS_OK;
    (*nfa_dm_cb.p_dm_cback) (NFA_DM_NOTIFY_ACT_FAIL_EVT, &dm_cback_data);
}
...
static void nfa_dm_nfc_response_cback (tNFC_RESPONSE_EVT event, tNFC_RESPONSE *p_data)
{
    ...
    case NFC_GEN_ERROR_REVT:                     /* generic error command or notification */
        /* CXD224x workaround: report CORE_GEN_ERROR */
        if( GetNumValue(NAME_CXD224X_WORKAROUND, &num, sizeof(num) ) && (num & NFC_CXD_WORKAROUND_GEN_ERROR_REPORT) )
        {
            dm_cback_data.status          = p_data->status;
            (*nfa_dm_cb.p_dm_cback) (NFA_DM_CORE_GEN_ERROR_REVT, &dm_cback_data);
        }
        break;
    ...
    case NFC_CORE_RESET_NTF_REVT: /* CORE_RESET_NTF event */
        /* CXD224x workaround: report CORE_CORE_RESET_NTF if NFC_SELECT_PANIC_ACTION_EVENT selected */
        dm_cback_data.status          = p_data->status;
        (*nfa_dm_cb.p_dm_cback) (NFA_DM_CORE_RESET_NTF_REVT, &dm_cback_data);
        break;
    default:
        break;
    }
...
BOOLEAN nfa_dm_enable (tNFA_DM_MSG *p_data)
{
    ...
    if (!(nfa_dm_cb.flags & NFA_DM_FLAGS_DM_IS_ACTIVE))
    {
        ...
        nfa_sys_cback_reg_hci_nv_write_event (nfa_dm_notify_nv_write_cback);/* CXD224x workaround: notify hci_nv_write */
        nfa_sys_cback_reg_hci_act_fail_event (nfa_dm_notify_act_fail_cback);/* CXD224x workaround: notify hci_act_fail */
...
static BOOLEAN nfa_dm_deactivate_polling (void)
{
    ...
    else if (nfa_dm_cb.disc_cb.disc_state == NFA_DM_RFST_POLL_ACTIVE)
    {
        ...
        {
            /* NFA P2P will deactivate NFC link after deactivating LLCP link */
            if (nfa_p2p_deactivate_llcp () == FALSE)
            {
                /* workaround:
                   accept NFA_DisablePolling() during RFST_DISCOVERY
                   when calling NFA_DisablePolling() during RFST_DISCOVERY,
                   LLCP_DeactivateLink() may fail and not send NFA_POLL_DISABLED_EVT,
                   by sending deactivate(idle) can be recover from this issue
                */
                NFA_TRACE_DEBUG0 ("nfa_dm_deactivate_polling () _deactivate_llcp false");
                nfa_dm_rf_deactivate (NFA_DEACTIVATE_TYPE_IDLE);
            }
        }
        else
...
BOOLEAN nfa_dm_act_stop_rf_discovery (tNFA_DM_MSG *p_data)
{
    ...
    else
    {
        if (nfa_dm_cb.flags & NFA_DM_FLAGS_EXCL_RF_ACTIVE)
        {
            /* LIBNCI bugfix: NO notify discovery stopped event.
               if exclusive RF control is running,
               Release exclusive RF control and notify deactivation to application before deactivate.
            */
...
char *nfa_dm_nfc_revt_2_str (tNFC_RESPONSE_EVT event)
{
    ...
    case NFC_CORE_RESET_NTF_REVT:
        return "NFC_CORE_RESET_NTF_REVT";
...
/*******************************************************************************
**
** Function         nfa_dm_act_send_halcc
**
** Description      Send the HAL Control command to the HAL
**
** Returns          FALSE (message buffer is NOT freed by caller)
**
*******************************************************************************/
BOOLEAN nfa_dm_act_send_halcc(tNFA_DM_MSG *p_data)
{
    BT_HDR  *p_cmd = (BT_HDR *)p_data;

    p_cmd->offset   = sizeof (tNFA_DM_API_SEND_VSC) - BT_HDR_SIZE;
    p_cmd->len      = p_data->send_vsc.cmd_params_len;
    NFC_SendHalControlCommand (p_data->send_vsc.oid, p_cmd, p_data->send_vsc.p_cback);

    /* Most dm action functions return TRUE, so nfa-sys frees the GKI buffer carrying the message,
     * This action function re-use the GKI buffer to send the HALCC, so the GKI buffer can not be freed by nfa-sys */
    return (FALSE);
}
