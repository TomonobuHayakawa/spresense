...
static tNFA_STATUS nfa_dm_set_rf_listen_mode_config (tNFA_DM_DISC_TECH_PROTO_MASK tech_proto_mask)
{
    ...
    */
    ...
    {
        /* LIBNCI: configurable LA_BIT_FRAME_SDD(Byte 1 of SENS_RES) for DH start */
        UINT8 frame_sdd=0;
        num = 0;
        if( GetNumValue(NAME_NFA_DM_LA_BIT_FRAME_SDD, &num, sizeof(num)) && num )
        {
            frame_sdd = (UINT8 )(num & 0xff);
        }
        else
        {
            frame_sdd = 0x04; /* orignal value */
        }
        UINT8_TO_STREAM (p, NFC_PMID_LA_BIT_FRAME_SDD);
        UINT8_TO_STREAM (p, NCI_PARAM_LEN_LA_BIT_FRAME_SDD);
        UINT8_TO_STREAM (p, frame_sdd); /* LIBNCI: configurable LA_BIT_FRAME_SDD(Byte 1 of SENS_RES) for DH end */
    ...
    if ((tech_proto_mask & (NFA_DM_DISC_MASK_LF_NFC_DEP|NFA_DM_DISC_MASK_LFA_NFC_DEP)) &&
        !nfa_dm_is_p2p_paused() )  /* LIBNCI bugfix: accept LFA only */
    {
...
static void nfa_dm_set_rf_listen_mode_raw_config (tNFA_DM_DISC_TECH_PROTO_MASK *p_disc_mask)
{
    ...
    */
        ...
        &&(p_cfg->lf_enable)  )
    {
        ...
        {
            UINT16_TO_BE_STREAM(p, p_cfg->lf_t3t_flags);
        }
        else
        {
...
void nfa_dm_start_rf_discover (void)
{
    ...
    if (num_params)
    {
        ...
        */
        ...
        if ((nfa_dm_cb.disc_cb.disc_state == NFA_DM_RFST_IDLE) && !(nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_NOTIFY))
        {
            /* if send  NFC_Deactivate(IDLE) from disc_sm, start_rf_discover is rollbacked after DEACTIVATE_RSP */
            NFA_TRACE_DEBUG1 ("nfa_dm_start_rf_discover(): detect rollback, flags=0x%x", nfa_dm_cb.disc_cb.disc_flags);
            nfa_dm_cb.disc_cb.disc_flags |= NFA_DM_DISC_FLAGS_W4_ROLLBACK;
        }
        else if (nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_W4_ROLLBACK)
        {
            /* during rollback is pendding, reject NFC_DiscoveryStart(), and restart after rollback is complated */
            NFA_TRACE_DEBUG1 ("nfa_dm_start_rf_discover(): enable recovery, flags=0x%x", nfa_dm_cb.disc_cb.disc_flags);
            nfa_dm_cb.disc_cb.disc_flags |= NFA_DM_DISC_FLAGS_RECOVERY;
            return;
        }
                                 ...
                                 tNFA_DM_RF_DISC_DATA *p_data)
{
    ...
    case NFA_DM_RF_DISCOVER_RSP:
        ...
        if (p_data->nfc_discover.status == NFC_STATUS_OK)
        {
            ...
            if (nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_RECOVERY)
            {
                /* restart rf_discover it recv during rollback */
                nfa_dm_cb.disc_cb.disc_flags &= ~NFA_DM_DISC_FLAGS_RECOVERY;
                NFA_TRACE_DEBUG2 ("%s: restart rf_discover, flags=0x%x", __FUNCTION__, nfa_dm_cb.disc_cb.disc_flags);
                nfa_dm_start_rf_discover ();
                break;
            }
                                        ...
                                        tNFA_DM_RF_DISC_DATA *p_data)
{
    ...
    case NFA_DM_RF_DEACTIVATE_RSP:
        ...
        if (!(nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_W4_NTF))
        {
            ...
            /* LIBNCI bugfix: P2P Deactivate CMD/RSP race condition start */
            NFA_TRACE_DEBUG3 ( "%s: deactivate.status=%x deactivate.type:0x%x", __FUNCTION__,
                               p_data->nfc_discover.deactivate.status, p_data->nfc_discover.deactivate.type);
            if (p_data->nfc_discover.deactivate.type == NFC_DEACTIVATE_TYPE_DISCOVERY)
            {
                /* not RF_discovery state, force to idle */
                nfa_dm_cb.disc_cb.disc_flags |= NFA_DM_DISC_FLAGS_W4_RSP;
                nfa_dm_disc_new_state (NFA_DM_RFST_IDLE);
                NFC_Deactivate (NFA_DEACTIVATE_TYPE_IDLE);
                if (p_data->nfc_discover.deactivate.status != NFC_STATUS_OK)
                {
                    break; /* send notify after recv Deactivate rsp */
                }
                /* in rare case if the state is already idle by DISCOVERY,
                   NFC_Deactivate(idle) will be failed, start start_rf_discover now */
            }
            /* LIBNCI bugfix: P2P Deactivate CMD/RSP race condition end */
            deact.status = NFC_STATUS_OK;
