...
{
    nfa_sys_init(); /* alloc memory before NFA_TRACE_API0 */
...
/*******************************************************************************
**
** Function         NFA_Destroy
**
** Description      This function destroys control blocks for NFA
**
**
** Returns          none
**
*******************************************************************************/
void NFA_Destroy(void)
{
    NFA_TRACE_API0 ("NFA_Destroy ()");
...
tNFA_STATUS NFA_EnablePolling (tNFA_TECHNOLOGY_MASK poll_mask)
{
...
#if (NFCNCI_DEBUGFUNC == TRUE)
    {
        ...
        int len = property_get("nfc.debug.poll_mask", valueStr, "");
        if( len > 0 )
        {
            unsigned int num;
            if( sscanf (valueStr, "0x%x", &num) == 1 ||
                sscanf (valueStr, "%u", &num) == 1 )
            {
                num &= NFA_TECHNOLOGY_MASK_ALL;
                poll_mask &= num ;
                NFA_TRACE_DEBUG2("%s: nfc.debug.poll_mask=0x%X", __FUNCTION__, num);
            }
        }
    }
#endif
...
tNFA_STATUS NFA_SetP2pListenTech (tNFA_TECHNOLOGY_MASK tech_mask)
{
    ...
    NFA_TRACE_API1 ("NFA_SetP2pListenTech (): tech_mask:0x%X", tech_mask);

#if (NFCNCI_DEBUGFUNC == TRUE)
    {

        char valueStr [PROPERTY_VALUE_MAX] = {0};
        int len = property_get("nfc.debug.p2plisten_mask", valueStr, "");
        if( len > 0 )
        {
            unsigned int num;
            if( sscanf (valueStr, "0x%x", &num) == 1 ||
                sscanf (valueStr, "%u", &num) == 1 )
            {
                num &= NFA_TECHNOLOGY_MASK_ALL;
                tech_mask &= num ;
                NFA_TRACE_DEBUG2("%s: nfc.debug.p2plisten_mask=0x%X", __FUNCTION__, num);
            }
        }
    }
#endif
...
**
**                  id              - The command id
**                  cmd_params_len  - The command parameter length
**                  p_cmd_params    - The command parameters
**                  p_cback         - The callback function to receive the command
**                                    status
**
** Returns          NFA_STATUS_OK if successfully initiated
                                                       ...
                                                       tNFA_HALCC_CBACK *p_cback)
{
    ...
    NFA_TRACE_API1 ("NFA_SendHalControlCommand() id=0x%x", id);
    ...
    if ((p_msg = (tNFA_DM_API_SEND_VSC *) GKI_getbuf (size)) != NULL)
    {
        ...
        p_msg->oid              = id;
        p_msg->p_cback          = p_cback;
        if (cmd_param_len && p_cmd_params)
        {
            p_msg->cmd_params_len   = cmd_param_len;
            p_msg->p_cmd_params     = (UINT8 *)(p_msg + 1);
            memcpy (p_msg->p_cmd_params, p_cmd_params, cmd_param_len);
        }
        else
        {
            p_msg->cmd_params_len   = 0;
