...
#else
#include <stdlib.h>
tNFA_SYS_CB *nfa_sys_cb_ptr=NULL;
#endif
...
#include <assert.h>
    nfa_sys_cb_ptr = (tNFA_SYS_CB *)GKI_os_malloc( sizeof(tNFA_SYS_CB) );
    assert (nfa_sys_cb_ptr);
#endif
...
/*******************************************************************************
**
** Function         nfa_sys_destroy
**
** Description      NFA destroy; remove allocated memory
**
**
** Returns          void
**
*******************************************************************************/
void nfa_sys_destroy (void)
{
#if NFA_DYNAMIC_MEMORY == TRUE
    if (nfa_sys_cb_ptr)
        GKI_os_free (nfa_sys_cb_ptr);
    nfa_sys_cb_ptr = NULL;
#endif
}
...
/* CXD224x workaround: Low Power control */
/*******************************************************************************
**
** Function        send_snooze_cmd
**
** Description      CXD224x workaround: Low Power control.
**                  send snooze cmd to HAL layer
**
** Returns          status
**
*******************************************************************************/
#include "config.h"
tNFA_STATUS nfa_sys_send_snooze_cmd (UINT8 param)
{
    unsigned int num=0;

    NFA_TRACE_DEBUG1 ("nfa_sys_send_snooze_cmd() param=%d", param);

    if(param != NFC_HAL_SENDCMD_SNOOZE_OP_DEFAULT
    && param != NFC_HAL_SENDCMD_SNOOZE_OP_LP_LONG_TIMER)
        return (NFA_STATUS_FAILED);

#if (defined (NFA_HALCC_INCLUDED) && (NFA_HALCC_INCLUDED==TRUE))
    if (GetNumValue(NAME_CXD224X_WORKAROUND, &num, sizeof(num)) && (num & NFC_CXD_WORKAROUND_LP_AFT_HCI_CPL))
    {
        UINT8 cmd_param_len=1;
        tNFA_DM_API_SEND_VSC *p_msg;
        UINT16  size = sizeof(tNFA_DM_API_SEND_VSC) + cmd_param_len; /*use VSC struct*/
        if ((p_msg = (tNFA_DM_API_SEND_VSC *) GKI_getbuf (size)) != NULL)
        {
            BT_HDR  *p_cmd;

            p_msg->hdr.event        = NFA_DM_API_HALCC_EVT;
            p_msg->oid              = NFC_HAL_SENDCMD_SNOOZE_ID;
            p_msg->p_cback          = NULL;
            p_msg->cmd_params_len   = cmd_param_len;
            p_msg->p_cmd_params     = (UINT8 *)(p_msg + 1);
            memcpy (p_msg->p_cmd_params, &param, cmd_param_len);
            p_cmd = (BT_HDR *)p_msg;
...
void nfa_sys_enable_subsystems (void)
{
    ...
    /* CXD224x workaround: Low Power control keeps wake */
    nfa_sys_send_snooze_cmd(NFC_HAL_SENDCMD_SNOOZE_OP_LP_LONG_TIMER);
