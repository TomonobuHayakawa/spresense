...
#define NFA_HCI_STATE_WAIT_NETWK_ENABLE_PROP 0x10    /* HCI is waiting for initialization of other host in the network */
                                                     /* with checking the change of the session id(prop) */
#define NFA_HCI_STATE_WAIT_DISCONNECT       0x11     /* waiting for NFC_ConnClose response when NFA_HciDisconnect called,*/
                                                     /* add for NFA_HCIDISCONNECT_INCLUDED */
#define NFA_HCI_STATE_APP_DELETEGATEANDPIPE 0x12     /* Removing all pipes and gates */
...
enum
{
    ...
    NFA_HCI_API_DISCONNECT_EVT,                                   /* Disconnect, add for NFA_HCIDISCONNECT_INCLUDED */
    NFA_HCI_API_DELETEGATEANDPIPE_EVT,                            /* Delete all gates and pipes */
    ...
    NFA_HCI_CHECK_TIMEOUT_EVT,                                    /* Timeout to check the change of the session id(prop) */
...
/* data type for NFA_HCI_API_DISCONNECT, add for NFA_HCIDISCONNECT_INCLUDED*/
typedef struct
{
    BT_HDR              hdr;
    tNFA_HANDLE         hci_handle;
    tNFA_STATUS         status;
} tNFA_HCI_API_DISCONNECT;
...
typedef union
{
    ...
    tNFA_HCI_API_DISCONNECT             disconnect;                     /* Disconnect, add for NFA_HCIDISCONNECT_INCLUDED*/
...
typedef struct
{
    ...
    TIMER_LIST_ENT                  pr_timer;                           /* Timer to avoid indefinitely waiting for prop session id read */
    ...
    UINT16                          intf_fail_count;                    /* CXD224x workaround: counter for ee_intf_activate_fail */
...
extern void nfa_hci_api_disconnect (tNFA_HCI_EVENT_DATA *p_evt_data); /* add for NFA_HCIDISCONNECT_INCLUDED */
extern void nfa_hci_api_deletegateandpipe (tNFA_HCI_EVENT_DATA *p_evt_data); /* add for NFA_HCIDISCONNECT_INCLUDED */
...
extern void nfa_hci_destroy (void); /* add for NFA_DYNAMIC_MEMORY == TRUE */
