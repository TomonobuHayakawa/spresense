...
enum
{
    ...
    NFA_DM_API_HALCC_EVT,           /* add for HALCC message pipe */
    NFA_DM_MAX_EVT
};
...
#define NFA_DM_DISC_FLAGS_W4_ROLLBACK    0x0200    /* Wait for discovery rollback via idle         */
#define NFA_DM_DISC_FLAGS_RECOVERY       0x0400    /* Enable recovery during rollback in progress  */
...
void nfa_ce_destroy (void);
#else
#define nfa_ce_init()
#define nfa_ce_destroy()
#endif /* (defined (NFC_NFCCE_INCLUDED) && (NFC_NFCCE_INCLUDED==TRUE)) */
...
void nfa_dm_destroy (void);
#if (defined (NFA_P2P_INCLUDED) && (NFA_P2P_INCLUDED==TRUE))
void nfa_p2p_init (void);
void nfa_p2p_destroy (void);
#else
#define nfa_p2p_init(void)
#define nfa_p2p_destroy(void)
...
void nfa_cho_destroy (void);
#else
#define nfa_cho_init(void)
#define nfa_cho_destroy(void)
...
void nfa_snep_destroy (void);
#else
#define nfa_snep_init(is_dta_mode)
#define nfa_snep_destroy(void)
#endif
...
#else
#define nfa_ee_init(void)
#define nfa_ee_destroy(void)
#define nfa_hci_init(void)
#define nfa_hci_destroy(void)
#endif
...
BOOLEAN nfa_dm_act_send_halcc (tNFA_DM_MSG *p_data); /* add for HALCC message pipe*/
