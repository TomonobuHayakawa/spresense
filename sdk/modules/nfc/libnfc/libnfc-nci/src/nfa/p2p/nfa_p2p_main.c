...
#else
#include <stdlib.h>
tNFA_P2P_CB *nfa_p2p_cb_ptr=NULL;
#endif
...
**
*******************************************************************************/
BOOLEAN nfa_p2p_deactivate_llcp (void)
{
    BOOLEAN ret;
    /* workaround: accept NFA_DisablePolling() during RFST_DISCOVERY */
    /* changes: return DeactivateLink status */
    ret = (LLCP_DeactivateLink () == LLCP_STATUS_SUCCESS) ? TRUE : FALSE;
    P2P_TRACE_DEBUG1 ("nfa_p2p_deactivate_llcp () ret=%d", ret);
    return ret;
}
...
#include <assert.h>
    nfa_p2p_cb_ptr = (tNFA_P2P_CB *)GKI_os_malloc( sizeof(tNFA_P2P_CB) );
    assert (nfa_p2p_cb_ptr);
#endif

    NFA_TRACE_DEBUG0 ("nfa_p2p_init ()");
...
/*******************************************************************************
**
** Function         nfa_p2p_destroy
**
** Description      Destroy NFA P2P
**
**
** Returns          None
**
