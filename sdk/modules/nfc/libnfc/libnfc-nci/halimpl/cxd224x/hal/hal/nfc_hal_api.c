...
/******************************************************************************
 *
 *  NFC Hardware Abstraction Layer API: Implementation for NFC
 *  controllers
 *
 ******************************************************************************/
#include <string.h>
...
void HAL_NfcInitialize (void)
{
   ...
   HAL_TRACE_API1 ("HAL_NfcInitialize (): NFC_HAL_TASK id=%i", NFC_HAL_TASK);
    ...
    GKIH_create_task ((TASKPTR)nfc_hal_main_task,
                     NFC_HAL_TASK,
                     NFC_HAL_TASK_STR,
...
void HAL_NfcOpen (tHAL_NFC_CBACK *p_hal_cback, tHAL_NFC_DATA_CBACK *p_data_cback)
{
    ...
    {
        nfc_hal_dm_init ();
        nfc_hal_cb2.p_stack_cback = p_hal_cback;
        nfc_hal_cb2.p_data_cback  = p_data_cback;

        /* Send startup event to NFC_HAL_TASK */
        GKIH_send_event (NFC_HAL_TASK, NFC_HAL_TASK_EVT_INITIALIZE);
    }
}
...
void HAL_NfcCoreInitialized (UINT8 *p_core_init_rsp_params)
{
    ...
    if ((p_msg = (NFC_HDR *)GKIH_getbuf ((UINT16)(size + NFC_HDR_SIZE))) != NULL)
    {
...
/******************************************************************************
***
** Function         HAL_HalCCMsgWrite
**
** Description      Send an HAL Control Command message to the HAL.
**
** Returns          void
**
*******************************************************************************/
static void HAL_HalCCMsgWrite (UINT16 data_len, UINT8 *p_data)
{
    NFC_HDR *p_msg;
    UINT8 mt;

    HAL_TRACE_API1 ("HAL_HalCCMsgWrite () len=%d", data_len);

    if (data_len > (NCI_MAX_CTRL_SIZE + NCI_MSG_HDR_SIZE))
    {
        HAL_TRACE_ERROR1 ("HAL_ExHalWrite (): too many bytes (%d)", data_len);
        return;
    }
...
void HAL_NfcWrite (UINT16 data_len, UINT8 *p_data)
{
    ...
    if( p_data[0] == (NCI_MTS_HALMSG|NCI_GID_PROP) )
    {
        HAL_HalCCMsgWrite( data_len, p_data );
        return;
    }
...
BOOLEAN HAL_NfcPreDiscover (void)
{
    ...
    HAL_TRACE_API1 ("HAL_NfcPreDiscover () -- pre_discover_done=%s", nfc_hal_cb2.pre_discover_done?"TRUE":"FALSE");

    if ((p_msg = (NFC_HDR *)GKIH_getpoolbuf (NFC_HAL_NCI_POOL_ID)) != NULL)
    {
        /* Send message to NFC_HAL_TASK: request to send CMD if registered */
        p_msg->event  = NFC_HAL_EVT_TO_PREDISCOVER_CMD_NFC_NCI;
        GKIH_send_msg (NFC_HAL_TASK, NFC_HAL_TASK_MBOX, p_msg);
    }

    if (nfc_hal_cb2.pre_discover_done == FALSE)
    {
        nfc_hal_cb2.pre_discover_done    = TRUE;
...
**
*******************************************************************************/
BOOLEAN  HAL_NfcPreDiscoverCmdReg (UINT16 data_len, UINT8 *p_data )
{
    BOOLEAN status = FALSE;
    NFC_HDR *p_msg;
    /* Send message to NFC_HAL_TASK */
    if ((p_msg = (NFC_HDR *)GKIH_getpoolbuf (NFC_HAL_NCI_POOL_ID)) != NULL)
    {
        p_msg->event  = NFC_HAL_EVT_TO_PREDISCOVER_CMD_REG;
        p_msg->offset = 0;
        p_msg->len    = data_len;
        memcpy( (UINT8 *) (p_msg + 1) + p_msg->offset, p_data, data_len );
        GKIH_send_msg (NFC_HAL_TASK, NFC_HAL_TASK_MBOX, p_msg);
    ...
    HAL_TRACE_API1 ("HAL_NfcPreDiscoverCmdReg data_len=%d", data_len);
    return status;
}
...
void HAL_NfcPowerCycle (void)
{
    ...
    if (nfc_hal_cb2.p_stack_cback)
    {
        ...
        GKIH_send_event (NFC_HAL_TASK, NFC_HAL_TASK_EVT_POWER_CYCLE);
    }
}
...
{
    HAL_TRACE_API1 ("HAL_NfcGetMaxNfcee: %d",nfc_hal_cb2.max_ee);
    return nfc_hal_cb2.max_ee;
}
