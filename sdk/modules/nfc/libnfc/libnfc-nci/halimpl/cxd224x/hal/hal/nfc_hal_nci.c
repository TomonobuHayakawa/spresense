...
{
    NFC_HDR *p_msg = nfc_hal_cb2.ncit_cb.p_rcv_msg;
    UINT8 u8;
    UINT8 *p, *pp;
    UINT8 hdr[2];
    UINT8   *ps, *pd;
    ...
    {
        if (nfc_hal_cb2.ncit_cb.p_frag_msg->layer_specific != p_msg->layer_specific)
        {
            /* check if these fragments are of the same NCI message */
            HAL_TRACE_ERROR2 ("nfc_hal_nci_assemble_nci_msg() - different messages 0x%x, 0x%x!!", nfc_hal_cb2.ncit_cb.p_frag_msg->layer_specific, p_msg->layer_specific);
            nfc_hal_cb2.ncit_cb.nci_ras  |= NFC_HAL_NCI_RAS_ERROR;
        }
        else if (nfc_hal_cb2.ncit_cb.nci_ras == 0)
        {
            disp_again = TRUE;
            /* if not previous reassembly error, append the new fragment */
            p_msg->offset   += NCI_MSG_HDR_SIZE;
            p_msg->len      -= NCI_MSG_HDR_SIZE;
            size    = GKIH_get_buf_size (nfc_hal_cb2.ncit_cb.p_frag_msg);
            needed  = (NFC_HDR_SIZE + nfc_hal_cb2.ncit_cb.p_frag_msg->len + nfc_hal_cb2.ncit_cb.p_frag_msg->offset + p_msg->len);
            if (size >= needed)
            {
                /* the buffer for reassembly is big enough to append the new fragment */
                ps   = (UINT8 *) (p_msg + 1) + p_msg->offset;
                pd   = (UINT8 *) (nfc_hal_cb2.ncit_cb.p_frag_msg + 1) + nfc_hal_cb2.ncit_cb.p_frag_msg->offset + nfc_hal_cb2.ncit_cb.p_frag_msg->len;
                memcpy (pd, ps, p_msg->len);
                nfc_hal_cb2.ncit_cb.p_frag_msg->len  += p_msg->len;
                /* adjust the NCI packet length */
                pd   = (UINT8 *) (nfc_hal_cb2.ncit_cb.p_frag_msg + 1) + nfc_hal_cb2.ncit_cb.p_frag_msg->offset + 2;
                *pd  = (UINT8) (nfc_hal_cb2.ncit_cb.p_frag_msg->len - NCI_MSG_HDR_SIZE);
            }
            else
            {
                nfc_hal_cb2.ncit_cb.nci_ras  |= NFC_HAL_NCI_RAS_TOO_BIG;
                HAL_TRACE_ERROR2 ("nfc_hal_nci_assemble_nci_msg() buffer overrun (%d + %d)!!", nfc_hal_cb2.ncit_cb.p_frag_msg->len, p_msg->len);
            }
        }
    ...
    {
        nfc_hal_cb2.ncit_cb.p_frag_msg = p_msg;
    }
...
#ifdef DISP_NCI
        if ((nfc_hal_cb2.ncit_cb.nci_ras == 0) && (disp_again))
        {
            DISP_NCI ((UINT8 *) (p_msg + 1) + p_msg->offset, (UINT16) (p_msg->len), TRUE);
        }
#endif
        /* clear the error flags, so the next NCI packet is clean */
        nfc_hal_cb2.ncit_cb.nci_ras = 0;
    }
    else
    {
        /* still reassembling */
    ...
    nfc_hal_cb2.ncit_cb.p_rcv_msg = p_msg;
}
...
static BOOLEAN nfc_hal_nci_receive_nci_msg (tNFC_HAL_NCIT_CB *p_cb, UINT8 byte)
{
    ...
    case NFC_HAL_RCV_NCI_MSG_ST:
        ...
        if ((p_cb->p_rcv_msg = (NFC_HDR *) GKIH_getpoolbuf (NFC_HAL_NCI_POOL_ID)) != NULL)
        {
            /* Initialize NFC_HDR */
...
{
    tNFC_HAL_NCIT_CB *p_cb = &(nfc_hal_cb2.ncit_cb);
    BOOLEAN msg_received = FALSE;

    /* because of no HCIT_TYPE_NFC, force  MSG_ST */
...
BOOLEAN nfc_hal_nci_preproc_rx_nci_msg (NFC_HDR *p_msg)
{
    ...
    if (nfc_hal_cb2.dev_cb.initializing_state != NFC_HAL_INIT_STATE2_IDLE)
    {
    ...
    else
    {
        ...
        else if (gid == NCI_GID_RF_MANAGE)
        {
            if (mt == NCI_MT_NTF)
            {
                ...
                {
                    if ((nfc_hal_cb2.max_rf_credits) && (payload_len > 5))
                    {
                        /* API used wants to limit the RF data credits */
                        p += 5; /* skip RF disc id, interface, protocol, tech&mode, payload size */
                        if (*p > nfc_hal_cb2.max_rf_credits)
                        {
                            HAL_TRACE_DEBUG2 ("RfDataCredits %d->%d", *p, nfc_hal_cb2.max_rf_credits);
                            *p = nfc_hal_cb2.max_rf_credits;
                        }
                    }
                }
#if (defined (NFC_HAL_DELAYED_NTF_INCLUDED) && NFC_HAL_DELAYED_NTF_INCLUDED==TRUE)
                if (op_code == NCI_MSG_RF_EE_DISCOVERY_REQ)
                {
                    /* save specific NTF packtes */
	...
	else if (gid == NCI_GID_EE_MANAGE)
	{
	    if (mt == NCI_MT_RSP)
            {
		HAL_TRACE_DEBUG2 ("EE_MANGE_RSP %x:%x", p[0], p[1]);
	    }
	}
    }
    ...
    if (nfc_hal_cb2.dev_cb.power_mode == NFC_HAL_POWER_MODE_FULL)
    {
        ...
        {
            /* extend idle timer */
            nfc_hal_dm_power_mode_execute (NFC_HAL_LP_RX_DATA_EVT2);
        }
    }
...
void nfc_hal_nci_send_cmd (NFC_HDR *p_buf)
{
    ...
    UINT8   nci_ctrl_size = nfc_hal_cb2.ncit_cb.nci_ctrl_size;
    UINT8   delta = 0;

    /* check low power mode state */
    continue_to_process = nfc_hal_dm_power_mode_execute (NFC_HAL_LP_TX_DATA_EVT2);

    if (!continue_to_process)
    {
        /* save the command to be sent until NFCC is free. */
        nfc_hal_cb2.ncit_cb.p_pend_cmd   = p_buf;
        return;
    }
...
void nfc_hal_nci_cmd_timeout_cback (void *p_tle)
{
    ...
    {
        if (nfc_hal_cb2.dev_cb.initializing_state <= NFC_HAL_INIT_STATE2_W4_PATCH_INFO)
        {
            NFC_HAL_SET_INIT_STATE2 (NFC_HAL_INIT_STATE2_IDLE);
            nfc_hal_main_pre_init_done (HAL_NFC_STATUS_ERR_CMD_TIMEOUT);
        }
        else if (nfc_hal_cb2.dev_cb.initializing_state == NFC_HAL_INIT_STATE2_W4_APP_COMPLETE)
        {
            if (nfc_hal_cb2.prm.state != NFC_HAL_PRM_ST2_IDLE)
            {
        ...
        else if (nfc_hal_cb2.dev_cb.initializing_state == NFC_HAL_INIT_STATE2_W4_POST_INIT_DONE)
        {
            NFC_HAL_SET_INIT_STATE2 (NFC_HAL_INIT_STATE2_IDLE);
            nfc_hal_cb2.p_stack_cback (HAL_NFC_POST_INIT_CPLT_EVT, HAL_NFC_STATUS_ERR_CMD_TIMEOUT);
        }
        else if (nfc_hal_cb2.dev_cb.initializing_state == NFC_HAL_INIT_STATE2_W4_CONTROL_DONE)
        {
            NFC_HAL_SET_INIT_STATE2(NFC_HAL_INIT_STATE2_IDLE);
            nfc_hal_cb2.p_stack_cback (HAL_NFC_RELEASE_CONTROL_EVT, HAL_NFC_STATUS_ERR_CMD_TIMEOUT);
        }
        else if (nfc_hal_cb2.dev_cb.initializing_state == NFC_HAL_INIT_STATE2_W4_PREDISCOVER_DONE)
        {
            NFC_HAL_SET_INIT_STATE2(NFC_HAL_INIT_STATE2_IDLE);
            nfc_hal_cb2.p_stack_cback (HAL_NFC_PRE_DISCOVER_CPLT_EVT, HAL_NFC_STATUS_ERR_CMD_TIMEOUT);
        }
    }
}
...
{
    HAL_TRACE_DEBUG2 ("HAL_NfcSetMaxRfDataCredits %d->%d", nfc_hal_cb2.max_rf_credits, max_credits);
    nfc_hal_cb2.max_rf_credits   = max_credits;
}
