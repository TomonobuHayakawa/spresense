...
unsigned char h_appl_trace_level = BT_TRACE_LEVEL_DEBUG;
...
unsigned char InitializeGlobalAppLogLevel ()
{
    ...
    if (GetNumValue (NAME_APPL_TRACE_LEVEL, &num, sizeof(num)))
        h_appl_trace_level = (unsigned char) num;
    ...
    if (h_appl_trace_level == 0xFF)
        h_appl_trace_level = BT_TRACE_LEVEL_DEBUG;
    ALOGD ("%s: level=%u", __FUNCTION__, h_appl_trace_level);
    return h_appl_trace_level;
}
...
UINT32 InitializeProtocolLogLevel () {
    ...
    if (len > 0)
    {
        ...
        {
            ScrProtocolTraceFlag_Hal = 0;
        } else {
            ScrProtocolTraceFlag_Hal = ~0;
        }
    }

    return ScrProtocolTraceFlag_Hal;
}
