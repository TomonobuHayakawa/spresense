...
void delete_hal_non_volatile_store (bool forceDelete)
{
    ...
    if (snprintf (filename, sizeof(filename), "%s%u", fn.c_str(), DH_NV_BLOCK))
        if (remove (filename) !=0)
            ALOGD ("%s: Error deleting file=%s", __FUNCTION__, filename);
/*
    if (snprintf (filename, sizeof(filename), "%s%u", fn.c_str(), HC_F3_NV_BLOCK))
        if (remove (filename) !=0)
            ALOGD ("%s: Error deleting file=%s", __FUNCTION__, filename);
    if (snprintf (filename, sizeof(filename), "%s%u", fn.c_str(), HC_F4_NV_BLOCK))
        if (remove (filename) !=0)
            ALOGD ("%s: Error deleting file=%s", __FUNCTION__, filename);
    if (snprintf (filename, sizeof(filename), "%s%u", fn.c_str(), HC_F2_NV_BLOCK))
        if (remove (filename) !=0)
            ALOGD ("%s: Error deleting file=%s", __FUNCTION__, filename);
    if (snprintf (filename, sizeof(filename), "%s%u", fn.c_str(), HC_F5_NV_BLOCK))
        if (remove (filename) !=0)
            ALOGD ("%s: Error deleting file=%s", __FUNCTION__, filename);
*/
}
...
int verify_hal_non_volatile_store ()
{
    ...
    return (isValid == false || fileExist == false) ? 1 : 0;
}
#endif /*(defined(NFC_HAL_HCI_INCLUDED) && (NFC_HAL_HCI_INCLUDED == TRUE))*/
